...
// Client prediction
static F32 sMaxLatencyTicks = 0;       // Max latency prediction
...
static U32 sDirtySetMask = (ForceFieldObjectType | 
                            ...
                            TurretObjectType  |
                            StationObjectType    | SensorObjectType);
...
bool PlayerData::preload(bool server, char errorBuffer[256])
{
   ...
   U32 i;
   for ( i = 0; i < NumTableActionAnims; i++,dp++) {
   ...
   const char *skiName = StringTable->insert("ski");
   const char *standJumpName = StringTable->insert("standjump");
   for (int c = 0; c < actionCount; c++,dp++)
   {
      if (dp->name == lookName)
         lookAction = c;
      else if (dp->name == skiName)
         skiAction = c;
      else if (dp->name == standJumpName)
         standJumpAction = c;
   }
...
{
   return (action == JumpAnim || action == standJumpAction);
}

void PlayerData::consoleInit()
{
...
void PlayerData::initPersistFields()
{
   ...
   addField("LFootSoftSound",       TypeAudioProfilePtr, Offset(sound[LFootSoft],          PlayerData));
   addField("RFootSoftSound",       TypeAudioProfilePtr, Offset(sound[RFootSoft],          PlayerData));
   addField("LFootHardSound",       TypeAudioProfilePtr, Offset(sound[LFootHard],          PlayerData));
   addField("RFootHardSound",       TypeAudioProfilePtr, Offset(sound[RFootHard],          PlayerData));
   addField("LFootMetalSound",      TypeAudioProfilePtr, Offset(sound[LFootMetal],         PlayerData));
   addField("RFootMetalSound",      TypeAudioProfilePtr, Offset(sound[RFootMetal],         PlayerData));
   addField("LFootSnowSound",       TypeAudioProfilePtr, Offset(sound[LFootSnow],          PlayerData));
   addField("RFootSnowSound",       TypeAudioProfilePtr, Offset(sound[RFootSnow],          PlayerData));
   addField("LFootShallowSound",    TypeAudioProfilePtr, Offset(sound[LFootShallowSplash], PlayerData));
   addField("RFootShallowSound",    TypeAudioProfilePtr, Offset(sound[RFootShallowSplash], PlayerData));
   addField("LFootWadingSound",     TypeAudioProfilePtr, Offset(sound[LFootWading],        PlayerData));
   addField("RFootWadingSound",     TypeAudioProfilePtr, Offset(sound[RFootWading],        PlayerData));
   addField("LFootUnderwaterSound", TypeAudioProfilePtr, Offset(sound[LFootUnderWater],    PlayerData));
   addField("RFootUnderwaterSound", TypeAudioProfilePtr, Offset(sound[RFootUnderWater],    PlayerData));
   addField("LFootBubblesSound",    TypeAudioProfilePtr, Offset(sound[LFootBubbles],       PlayerData));
   addField("RFootBubblesSound",    TypeAudioProfilePtr, Offset(sound[RFootBubbles],       PlayerData));
...
Player::Player()
{
   ...
   mActionAnimation.time = ActionAnimation::Scale;
...
bool Player::onAdd()
{
   ...
   if (serverAnim.action != PlayerData::NullAnimation) {
      ...
      if (!isServerObject() && inSittingAnim())
         mMountPending = sMountPendingTickWait;
      else
         mMountPending = 0;
   }
...
void Player::processTick(const Move* move)
{
   ...
   if(isClientObject()) {
      if(conObj->mWaterCoverage >= 1.0f && mDamageState == Enabled)
      {
      ...
      // if(mImpactSound > mClientImpactSound)
      if(mImpactSound)
         playImpactSound();
   }
   else {
...
void Player::updateState()
{
   switch (mState) {
      case RecoverState:
         if (mRecoverTicks-- == 0) {
            if (mReversePending) {  // this serves and counter, and direction state 
...
void Player::updateMove(const Move* move)
{
   ...
   VectorF moveVec;
   F32 moveSpeed;
   if (mState == MoveState && mDamageState == Enabled && !mDisableMove)
   {
   ...
   if (runSurface) {
      ...
      // Force a 0 move if there is no energy
      VectorF pv;
   ...
   if (move->trigger[2] && !isMounted() && canJump()) 
   {
      ...
      if (zSpeedScale <= mDataBlock->maxJumpSpeed) {
         ...
         if (speed > sStandingJumpSpeed)
            jumpAction = PlayerData::JumpAnim;
         else
            jumpAction = mDataBlock->standJumpAction;
...
void Player::updateDeathOffsets()
{
   ...
   {
      // Get ground delta from the last time we offset this.
      F32      pos = deathDelta(mDeath.posAdd);
      mDeath.lastPos = pos;
   }
   else
      mDeath.clear();
...
bool Player::setArmThread(U32 action)
{
   ...
   if (action != mArmAnimation.action && anim.sequence != -1) {
...
void Player::updateActionThread()
{
   ...
   if( triggeredLeft || triggeredRight )
   {
      ...
      {
         S32 sound = -1;
         // Create foot puff on ground.  New emitter every time for visibility reasons
        ...
        (mActionAnimation.action != PlayerData::NullAnimation) )
   {
      ...
      F32 scale = 1;
      if (mActionAnimation.time == ActionAnimation::Scale && anim.speed) {
         VectorF vel;
         mWorldToObj.mulV(mVelocity,&vel);
          ...
          mFabs(distance.z) < mObjBox.len_z())
      {
         ...
         while (pList != &rList) {
            ...
            if (pConvex->getObject()->getTypeMask() & sCollisionMoveMask) {
               ...
               {
                  if (pConvex->getObject()->getTypeMask() & ForceFieldObjectType)
                  {
                     ForceFieldBare* pField = dynamic_cast<ForceFieldBare*>(pConvex->getObject());
                     if (pField == NULL || pField->isPermiableTo(this) == false)
      ...
      if (collisionList.count != 0 && collisionList.t < 1.0) {
         ...
         if (bd > mDataBlock->minImpactSpeed && !mMountPending) {
            ...
            if (mDamageState == Enabled && mState != RecoverState) {
               ...
               // Con::printf("  minImpact = %f, this one = %f", mDataBlock->minImpactSpeed, bd);
         ...
         if (!isGhost() && collision->object->getTypeMask() & ShapeBaseObjectType)
            queueCollision(static_cast<ShapeBase*>(collision->object));
      }
      else
      {
         totalMotion += (end - start).len();
...
void Player::findContact(bool* run,bool* jump,VectorF* contactNormal)
{
   ...
   wBox.min = pos + mObjBox.min - exp;
   wBox.max.x = pos.x + mObjBox.max.x;
   wBox.max.y = pos.y + mObjBox.max.y;
   wBox.max.z = pos.z + mObjBox.min.z + sTractionDistance;
   ...
   wBox.max.z = pos.z + mObjBox.max.z;
   ...
   while (pList != &rList) {
      ...
      if(objectMask & sDirtySetMask)
      {
         ...
         if(objectMask & ShapeBaseObjectType)
            static_cast<ShapeBase *>(pConvex->getObject())->setControlDirty();
      }
      ...
      if (objectMask & TriggerObjectType) {
         ...
         if (getWorldBox().isOverlapped(pConvex->getObject()->getWorldBox()))
            queueCollision(static_cast<ShapeBase*>(pConvex->getObject()));
      }
         ...
         if (plistBox.isOverlapped(convexBox))
         {
            if (serverParent == NULL) {
               if (objectMask & ForceFieldObjectType)
               {
                  ForceFieldBare* pField = dynamic_cast<ForceFieldBare*>(pConvex->getObject());
                  if (pField == NULL || pField->isPermiableTo(this) == false)
                     pConvex->getPolyList(&polyList);
   ...
   if (polyList.isEmpty() == false) {
      // Pick flatest surface
      F32 bestVd = -1;
...
void Player::getMuzzleTransform(U32 imageSlot,MatrixF* mat)
{
   ...
   if (getDamageState() == Enabled)
   {
      ...
      RayInfo rinfo;
      if (getContainer()->castRay(start, end, -1, &rinfo)) {
         Point3F finalPoint;
...
{
   if (mControlObject && mControlObject == getObjectMount()) {
...
bool Player::writePacketData(GameConnection *connection, BitStream *stream)
{
   ...
   if (mControlObject) {
      ...
      if (stream->writeFlag(gIndex != -1)) {
         ...
         ret = mControlObject->writePacketData(connection, stream);
      }
      else // the control object exists, but not on the other side...
         ret = false;
   }
   else
...
void Player::unpackUpdate(NetConnection *con, BitStream *stream)
{
   ...
   if (stream->readFlag()) {
      ...
      if (isProperlyAdded()) {
         ...
         mMountPending = (inSittingAnim() ? sMountPendingTickWait : 0);
      }
      else {
         mActionAnimation.action = action;
         mActionAnimation.holdAtEnd = hold;
   ...
   if (stream->readFlag()) {
      ...
      if (stream->readFlag() && isProperlyAdded()) {
         // Warp to the server's position over time
         // First advance pos to estimated server time.
         S32 ticks = mCeil(con->getRoundTripTime() / (2 * TickMs));
         if (ticks > sMaxLatencyTicks) {
            ticks = (S32)sMaxLatencyTicks;
         }
         if (ticks > 0) {
            Point3F orgPos,orgRot = mRot;
            getTransform().getColumn(3,&orgPos);

            setPosition(pos,rot);
            while (ticks--) {
               updateState();
               updateMove(&delta.move);
               updatePos();
            }
            getTransform().getColumn(3,&pos);

            setPosition(orgPos,orgRot);
         }
...
static const char* cGetState(SimObject *ptr, S32, const char **)
{
   Player* obj = static_cast<Player*>(ptr);
   return obj->getStateName();
}

static const char* cGetDamageLocation(SimObject *ptr, S32, const char **argv)
{
   Player *obj = static_cast<Player*>(ptr);
   ...
   obj->getDamageLocation(pos, buffer1, buffer2);
   
   dSprintf(buff, 128, "%s %s", buffer1, buffer2);
   return buff;
}

static bool cSetArmThread(SimObject *ptr, S32, const char **argv)
{
   Player* obj = static_cast<Player*>(ptr);
   return obj->setArmThread(argv[2]);
}

static bool cSetActionThread(SimObject *ptr, S32 argc, const char **argv)
{
   Player* obj = static_cast<Player*>(ptr);
   bool hold = (argc > 3)? dAtob(argv[3]): false;
   bool fsp  = (argc > 4)? dAtob(argv[4]): true;
   return obj->setActionThread(argv[2],hold,true,fsp);
}

static bool cSetControlObject(SimObject *ptr, S32, const char **argv)
{
   Player *obj = static_cast<Player*>(ptr);
   obj->setControlDirty();
...
static S32 cGetControlObject(SimObject *ptr, S32, const char **)
{
   Player *obj = static_cast<Player*>(ptr);
   ShapeBase* controlObject = obj->getControlObject();
   return controlObject? controlObject->getId(): 0;
}

static void cClearControlObject(SimObject *ptr, S32, const char **)
{
   Player *obj = static_cast<Player*>(ptr);
   obj->setControlObject(0);
}

static void cDisableMove(SimObject *ptr, S32, const char **argv)
{
   Player *obj = static_cast<Player*>(ptr);
   obj->disableMove(dAtob(argv[2]));
}

static void cSetPilot(SimObject *ptr, S32, const char **argv)
{
   Player *obj = static_cast<Player*>(ptr);
   obj->setPilot(dAtob(argv[2]));
}
...
static bool cCheckDismountPoint(SimObject *ptr, S32, const char **argv)
{
   ...
   return obj->checkDismountPosition(oldPosMat, posMat);
}
...
{
//    Con::addVariable("bingy", TypeS32, &bingy);
//    Con::addVariable("boingy", TypeS32, &boingy);
   ...
   Con::addVariable("Player::maxLatencyTicks",TypeF32,&sMaxLatencyTicks);

   Con::addCommand("Player", "getState", cGetState, "obj.getState()", 2, 2);
   Con::addCommand("Player", "setArmThread", cSetArmThread, "obj.setArmThread(sequenceName);", 3, 3);
   Con::addCommand("Player", "setActionThread", cSetActionThread, "obj.setActionThread(sequenceName,<hold>,<fsp>)", 3, 5);
   Con::addCommand("Player", "setControlObject", cSetControlObject, "obj.setControlObject(obj)", 3, 3);
   Con::addCommand("Player", "getControlObject", cGetControlObject, "obj.getControlObject()", 2, 2);
   Con::addCommand("Player", "clearControlObject", cClearControlObject, "obj.clearControlObject()", 2, 2);
   Con::addCommand("Player", "disableMove", cDisableMove, "obj.disableMove(bool)", 3, 3);
   Con::addCommand("Player", "setPilot", cSetPilot, "obj.setPilot(bool)", 3, 3);
   Con::addCommand("Player", "getDamageLocation", cGetDamageLocation, "obj.getDamageLocation(pos)", 3, 3);

   Con::addCommand("Player", "checkDismountPoint", cCheckDismountPoint, "obj.checkDismountPoint(\"x y z\", \"x y z\")", 4, 4);
}
   ...
   if(mWaterCoverage == 0.0f) {
      ...
      switch (sound) {
         case 0:
            //Soft
            if(triggeredLeft)
               alxPlay(mDataBlock->sound[PlayerData::LFootSoft], &footMat);
            else
               alxPlay(mDataBlock->sound[PlayerData::RFootSoft], &footMat);
         break;
         case 1:
            //Hard
            if(triggeredLeft)
               alxPlay(mDataBlock->sound[PlayerData::LFootHard], &footMat);
            else
               alxPlay(mDataBlock->sound[PlayerData::RFootHard], &footMat);
         break;
         case 2:
            //Metal
            if(triggeredLeft)
               alxPlay(mDataBlock->sound[PlayerData::LFootMetal], &footMat);
            else
               alxPlay(mDataBlock->sound[PlayerData::RFootMetal], &footMat);
         break;
         case 3:
            //Snow
            if(triggeredLeft)
               alxPlay(mDataBlock->sound[PlayerData::LFootSnow], &footMat);
            else
               alxPlay(mDataBlock->sound[PlayerData::RFootSnow], &footMat);
         break;
         default:
            //Hard
            if(triggeredLeft)
      ...
      if(mWaterCoverage < mDataBlock->footSplashHeight) {
         if(triggeredLeft)
            alxPlay(mDataBlock->sound[PlayerData::LFootShallowSplash], &footMat);
         else
            alxPlay(mDataBlock->sound[PlayerData::RFootShallowSplash], &footMat);
      }
      else if(mWaterCoverage < 1.0) {
         if(triggeredLeft)
            alxPlay(mDataBlock->sound[PlayerData::LFootWading], &footMat);
         else
            alxPlay(mDataBlock->sound[PlayerData::RFootWading], &footMat);
      }      
      else {
         if(triggeredLeft) {
            alxPlay(mDataBlock->sound[PlayerData::LFootUnderWater], &footMat);
            alxPlay(mDataBlock->sound[PlayerData::LFootBubbles], &footMat);
         }
         else {
            alxPlay(mDataBlock->sound[PlayerData::RFootUnderWater], &footMat);
            alxPlay(mDataBlock->sound[PlayerData::RFootBubbles], &footMat);
         }
      }      
   }
}
...
void Player::updateSplash()
{
   ...
   if( curPos.equal( mLastPos ) ) return;   
   ...
   if( pointInWater( curPos ) )
   {
      if( !pointInWater( mLastPos ) )
      {
         ...
         RayInfo rInfo;
         if( gClientContainer.castRay(mLastPos, curPos, WaterObjectType, &rInfo) )
         {
...
void Player::updateFroth( F32 dt )
{
   ...
   if( mBubbleEmitterTime < mDataBlock->bubbleEmitTime )
   {
      if( mSplashEmitter[PlayerData::BUBBLE_EMITTER] )
      {
         ...
         mSplashEmitter[emitNum]->emitParticles( mLastPos, emissionPoint, Point3F( 0.0, 0.0, 1.0 ), 
                                                 moveDir, dt * 1000.0 );
      }
   }
   ...
   for( i=0; i<PlayerData::BUBBLE_EMITTER; i++ )
   {
      if( mSplashEmitter[i] )
      {
         mSplashEmitter[i]->emitParticles( mLastWaterPos, contactPoint, Point3F( 0.0, 0.0, 1.0 ), 
                                           moveDir, emitRate );
      }
   }
...
void Player::disableMove(bool state)
{
   setControlDirty();
   mDisableMove = state;   
}

void Player::setPilot(bool state)
{
   mPilot = state;   
}

bool Player::isPilot() const
{
   return mPilot;   
}
