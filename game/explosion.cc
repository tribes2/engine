...
F32 cCalcCoverage(SimObject*, S32, const char** argv)
{
   Point3F pos, center;
...
void ExplosionData::initPersistFields()
{
   ...
   Con::registerType(TypeExplosionDataPtr, sizeof(ExplosionData*),
                     REF_GETDATATYPE(ExplosionData),
                     REF_SETDATATYPE(ExplosionData));
   
   addField("explosionShape",  TypeString,                  Offset(dtsFileName,     ExplosionData));
...
bool ExplosionData::onAdd()
{
   ...
   if( !shockwave && shockwaveID )
   {
      ...
      {
         Con::errorf( ConsoleLogEntry::General, "ExplosionData::onAdd: Invalid packet, bad datablockId(shockwave): 0x%x", shockwaveID );
      }
   }
...
void ExplosionData::packData(BitStream* stream)
{
   ...
   stream->write(offset);
   stream->writeFlag(shockwaveOnTerrain);
...
void ExplosionData::unpackData(BitStream* stream)
{
   ...
   stream->read(&offset);
   shockwaveOnTerrain = stream->readFlag();
...
bool ExplosionData::preload(bool server, char errorBuffer[256])
{
   ...
   if (dtsFileName && dtsFileName[0]) {
      char fullName[256];
      dSprintf(fullName,sizeof(fullName),"shapes/%s", dtsFileName);
      explosionShape = ResourceManager->load(fullName);
...
bool Explosion::onAdd()
{
   ...
   // start shockwave immediately - it has its own delay
   if( mDataBlock->shockwave )
   {
      if( !mDataBlock->shockwaveOnTerrain || (mCollideType & TerrainObjectType) )
      {
         MatrixF trans = getTransform();
         Shockwave* shockwave = new Shockwave;
         shockwave->onNewDataBlock( mDataBlock->shockwave );
         shockwave->setTransform( trans );
         shockwave->setInitialState( trans.getPosition(), mInitialNormal );
...
void Explosion::prepModelView(SceneState* state)
{
   ...
   if (mDataBlock->faceViewer == true) {
      targetVector = mInitialPosition - state->getCameraPosition();
...
void Explosion::launchDebris( Point3F &axis )
{
   ...
   for( int i=0; i<numDebris; i++ )
   {
      ...
      debris->onNewDataBlock( mDataBlock->debrisList[0] );
...
void Explosion::spawnSubExplosions()
{
   ...
   for( S32 i=0; i<ExplosionData::EC_MAX_SUB_EXPLOSIONS; i++ )
   {
      if( mDataBlock->explosionList[i] )
      {
         ...
         pExplosion->onNewDataBlock( mDataBlock->explosionList[i] );
...
bool Explosion::explode()
{
   ...
   if (mDataBlock->particleEmitter) {
      ...
      emitter->onNewDataBlock(mDataBlock->particleEmitter);
