      ...
      if (bd >= 0.0)
      {
         ...
         if (mDataBlock->sticky && rinfo.object->getType() & (InteriorObjectType|TerrainObjectType)) {
            ...
            if (isServerObject() && (typeMask & ShapeBaseObjectType))
               queueCollision(static_cast<ShapeBase*>(rinfo.object));
         }
      }
   }
   if (mCollisionObject)
...
void Item::unpackUpdate(NetConnection *connection, BitStream *stream)
{
   ...
   if (stream->readFlag()) {
      ...
      if (stream->readFlag() && isProperlyAdded()) {
         ...
         S32 ticks = (S32)(mCeil(connection->getRoundTripTime() / (2 * TickMs)));
         if (ticks > sMaxLatencyTicks)
            ticks = (S32)sMaxLatencyTicks;
         if (ticks > 0) {
            MatrixF orgPos = mObjToWorld;
            VectorF orgVel = mVelocity;
            bool orgRest   = mAtRest;
            ...
            while (ticks--) {
               updateVelocity(TickSec);
               updateWorkingCollisionSet(sClientCollisionMask, TickSec);
               updatePos(sClientCollisionMask, TickSec);
            }
            mObjToWorld.getColumn(3,&pos);

            Parent::setTransform(orgPos);
            mVelocity = orgVel;
            mAtRest = orgRest;
         }
         ...
         F32 dt = as? delta.warpOffset.len() / as: sMaxWarpTicks;
...
static bool cIsRotating(SimObject *ptr, S32, const char **)
{
   Item* obj = static_cast<Item*>(ptr);
   return obj->isRotating();
}

static bool cSetCollisionTimeout(SimObject *ptr, S32, const char **argv)
{
   Item* obj = static_cast<Item*>(ptr);
   ShapeBase* source;
   if (Sim::findObject(dAtoi(argv[2]),source)) {
      obj->setCollisionTimeout(source);
      return true;
   }
   return false;
}

static const char * cGetLastStickyPos(SimObject *ptr, S32, const char **)
{
   AssertFatal(dynamic_cast<Item*>(ptr) != NULL, "error, how did a non-item get in here?");
   Item* obj = static_cast<Item*>(ptr);

   char* ret = Con::getReturnBuffer(256);
   if (obj->isServerObject())
      dSprintf(ret, 255, "%g %g %g",
               obj->mStickyCollisionPos.x,
               obj->mStickyCollisionPos.y,
               obj->mStickyCollisionPos.z);
   else
      dStrcpy(ret, "0 0 0");
...
static const char * cGetLastStickyNormal(SimObject *ptr, S32, const char **)
{
   AssertFatal(dynamic_cast<Item*>(ptr) != NULL, "error, how did a non-item get in here?");
   Item* obj = static_cast<Item*>(ptr);
   ...
   if (obj->isServerObject())
      dSprintf(ret, 255, "%g %g %g",
               obj->mStickyCollisionNormal.x,
               obj->mStickyCollisionNormal.y,
               obj->mStickyCollisionNormal.z);
   else
      dStrcpy(ret, "0 0 0");
...
void Item::consoleInit()
{
   ...
   Con::addVariable("Item::maxLatencyTicks",TypeF32,&sMaxLatencyTicks);
}
