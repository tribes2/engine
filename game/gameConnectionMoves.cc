...
void Move::unclamp()
{
   yaw = S16(pyaw) * M_2PI / F32(0x10000);
   pitch = S16(ppitch) * M_2PI / F32(0x10000);
   roll = S16(proll) * M_2PI / F32(0x10000);
...
void Move::clamp()
{
   ...
   pyaw = U32((yaw / M_2PI) * 0x10000) & 0xFFFF;
   ppitch = U32((pitch / M_2PI) * 0x10000) & 0xFFFF;
   proll = U32((roll / M_2PI) * 0x10000) & 0xFFFF;
...
void GameConnection::collectMove(U32 time)
{
   ...
   {
      pushMove(mv);
      recordBlock(time + 1, BlockTypeMove, sizeof(Move), &mv);
   }
}
