...
void cLightningWarningFlashes(SimObject* obj, S32, const char**)
{
   AssertFatal(dynamic_cast<Lightning*>(obj) != NULL, "How did a non-lightning object get in here?");
   Lightning* lightning = static_cast<Lightning*>(obj);

   if (lightning->isServerObject())
      lightning->warningFlashes();
}

void cLightningStrikeRandomPoint(SimObject* obj, S32, const char**)
{
   AssertFatal(dynamic_cast<Lightning*>(obj) != NULL, "How did a non-lightning object get in here?");
   Lightning* lightning = static_cast<Lightning*>(obj);
...
void cLightningStrikeObject(SimObject* obj, S32, const char** argv)
{
   AssertFatal(dynamic_cast<Lightning*>(obj) != NULL, "How did a non-lightning object get in here?");
   Lightning* lightning = static_cast<Lightning*>(obj);
   ...
   if (lightning->isServerObject() && Sim::findObject(id, pSB))
      lightning->strikeObject(pSB);
}
...
void Lightning::consoleInit()
{
   Con::addCommand("Lightning", "warningFlashes",    cLightningWarningFlashes,    "[LightningObject].warningFlashes()",    2, 2);
   Con::addCommand("Lightning", "strikeRandomPoint", cLightningStrikeRandomPoint, "[LightningObject].strikeRandomPoint()", 2, 2);
   Con::addCommand("Lightning", "strikeObject",      cLightningStrikeObject,      "[LightningObject].strikeObject(id)",    3, 3);
}
...
void Lightning::processTick(const Move* move)
{
   ...
   if (isServerObject()) {
      S32 msBetweenStrikes = 60.0 / strikesPerMinute * 1000.0;
...
void Lightning::strikeObject(ShapeBase*)
{
   ...
   AssertFatal(false, "not yet done");
}
