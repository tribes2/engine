...
void setDetailSlider()
{
   ...
   slider->setField("range",avar("0 %f",-0.01f + (F32)numDL));
...
Vector<Platform::FileInfo> showFiles(__FILE__, __LINE__);

void showSetFileList(const char * path, const char * ext, const char * command)
{
   // set up showFileList dialog
   GuiTextListCtrl * list = static_cast<GuiTextListCtrl*>(Sim::findObject("showFileList"));
   if (!list)
      return;
      
   showFiles.clear();
   Platform::dumpPath(path, showFiles);

   // get rid of files with the wrong extension
   S32 i;
   for (i=showFiles.size()-1;i>=0;i--)
   {
      // get extension from showFiles[i]
      const char * fileExt = dStrrchr(showFiles[i].pFileName,'.');
      if (!fileExt || dStricmp(fileExt+1,ext))
         showFiles.erase(i);
   }

   list->clear();
   for (i=0;i<showFiles.size();i++)
      list->addEntry(i, showFiles[i].pFileName);
   if (showFiles.size())
      list->setSelectedCell(Point2I(0,0));
   else
      list->deselectCells();
      
   Con::setVariable("$showFileCommand",command);
}
...
ShowTSShape::ShowTSShape(const char * shapeName)
{
   ...
   stayGrounded = true;
   char fileBuffer[256];
   dStrcpy(fileBuffer,"shapes/");
   dStrcat(fileBuffer,shapeName);
   ...
   emapList.load(MeshTexture,true);
...
bool ShowTSShape::handleMovement(float leftSpeed, float rightSpeed,
                                 float forwardSpeed, float backwardSpeed,
                                 ...
                                 float delta)
{
   ...
   float turnLR = (gShowShapeRightSpeed-gShowShapeLeftSpeed) * delta * 0.1f;
   ...
   if (orbitDist<minOrbitDist)
      orbitDist = minOrbitDist;
   float invD = orbitDist>0.001f ? 1.0f / orbitDist : 0.0f;

   // up/down & left/right = rotate

   float xr = (upSpeed-downSpeed) * invD * delta;    // convert linear speed to angular
   float zr = (leftSpeed-rightSpeed) * invD * delta; // convert linear speed to angular
...
//   dglMultMatrix(&mat);
   ...
   if (Con::getBoolVariable("$showAutoDetail",false))
   {
      ...
      {
         char buffer[32];
         dSprintf(buffer,32,"%f",(F32)shapeInstance->getCurrentDetail()+1.0f-shapeInstance->getCurrentIntraDetail());
   ...
   else
   {
      ...
      if (slider)
         shapeInstance->setCurrentDetail(slider->getValue(),1.0f-(F32)((F32)slider->getValue()-(S32)slider->getValue()));
   }
   ...
   if (detailText1 && detailText2)
   {
      ...
      S32 dlSize = dl>=0 ? shapeInstance->getShape()->details[dl].size : 0;
      ...
      dSprintf(buffer,128,"detail leve: %i, detail size: %i",dl,dlSize);
      detailText1->setText(buffer);
      dSprintf(buffer,128,"size: %f, polys: %i, dist: %f",pixelSize,polys,dist);
...
{
   // do we have items ready to be loaded?
   if (!loadQueue.empty())
   {
      while (!loadQueue.empty())
      {
         char buffer[256];
         if (dStrstr( static_cast<const char*>( loadQueue.front() ), static_cast<const char*>( ".dts" ) ))
         {
            dSprintf(buffer,256,"showShapeLoad(\"%s\");",loadQueue.front());
            Con::evaluate(buffer);
         }
         else if (dStrstr( static_cast<const char*>( loadQueue.front() ), static_cast<const char*>( ".dsq" )))
         {
            dSprintf(buffer,256,"showSequenceLoad(\"%s\");",loadQueue.front());
            Con::evaluatef(buffer);
         }
         else if (dStrstr( static_cast<const char*>( loadQueue.front() ), static_cast<const char*>( ".mis" )))
            Con::setVariable("$showMission",loadQueue.front());
         dFree(loadQueue.front());
         loadQueue.pop_front();
      }
      Con::evaluate("startShow();");
   }
   ...
   if (currentShow && threadList && slider && slider->mAwake && threadList->getSelectedCell().y>=0)
   {
      ...
      {
         char buffer[32];
         dSprintf(buffer,32,"%f",currentShow->getPos(th));
         slider->setScriptValue(buffer);
      }
      else
...
static void cShowLoad(SimObject *, S32 argc, const char **argv)
{
   argc;
...
static void cShowSequenceLoad(SimObject *, S32 argc, const char ** argv)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowSelectSequence(SimObject *, S32, const char **)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
   ...
   if (threadNum>=0 && seq>=0)
   {
      ...
      {
         char buffer[32];
         dSprintf(buffer,32,"%f",currentShow->getPos(threadNum));
         slider->setScriptValue(buffer);
      }
   }
...
static void cShowSetDetailSlider(SimObject *, S32, const char **)
{
   setDetailSlider();
}

static void cShowUpdateThreadControl(SimObject *, S32, const char **)
{
   showUpdateThreadControl();
}

static void cShowPlay(SimObject *, S32 argc, const char ** argv)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;

   bool val = !dStricmp(argv[0],"showPlay");

   if (argc==1)
   {
      for (S32 i=0; i < currentShow->getThreadCount(); i++)
         currentShow->setPlay(i,val);
   }
   else
   {
      S32 i = dAtoi(argv[1]);
...
static void cShowSetScale(SimObject *, S32, const char ** argv)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowSetPos(SimObject *, S32, const char ** argv)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowNewThread(SimObject *, S32, const char **)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowDeleteThread(SimObject *, S32, const char ** argv)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowSetFileList(SimObject *, S32, const char ** argv)
{
   showSetFileList(argv[1],argv[2],argv[3]);
}

static void cShowToggleRoot(SimObject *, S32, const char **)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowToggleStick(SimObject *, S32, const char **)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
   if (!currentShow)
      return;
...
static void cShowSetCamera(SimObject *, S32, const char ** argv)
{
   ShowTSShape * currentShow = ShowTSShape::currentShow;
...
static void cShowSetKeyboard(SimObject *, S32, const char ** argv)
{
   keyboardControlsShape = (argv[1][0]=='t' || argv[1][0]=='T');
}

static void cShowTurnLeft(SimObject *, S32, const char **argv)
{
   gShowShapeLeftSpeed = dAtof(argv[1]);
}

static void cShowTurnRight(SimObject *, S32, const char **argv)
{
   gShowShapeRightSpeed = dAtof(argv[1]);
}

static void cShowSetLight(SimObject *, S32, const char **)
{
...
void ShowInit()
{
   Con::addCommand("showShapeLoad",cShowLoad,"showShapeLoad(shapeName,faceCamera);",2,3);
   Con::addCommand("showSequenceLoad",cShowSequenceLoad,"showSequenceLoad(sequenceFile,[sequenceName]);",2,3);
   
   Con::addCommand("showTurnLeft",cShowTurnLeft,"showTurnLeft(amt);",2,2);
   Con::addCommand("showTurnRight",cShowTurnRight,"showTurnRight(amt);",2,2);
   
   Con::addCommand("showUpdateThreadControl",cShowUpdateThreadControl,"showUpdateThreadControl();",1,1);
   Con::addCommand("showSelectSequence",cShowSelectSequence,"showSelectSequence();",1,1);
   
   Con::addCommand("showPlay",cShowPlay,"showPlay([threadNum]);",1,2);
   Con::addCommand("showStop",cShowPlay,"showPlay([threadNum]);",1,2);

   Con::addCommand("showSetScale",cShowSetScale,"showSetScale(threadNum,scale);",3,3);
   Con::addCommand("showSetPos",cShowSetPos,"showSetPos(threadNum,pos);",2,2);
   
   Con::addCommand("showNewThread",cShowNewThread,"showNewThread();",1,1);
   Con::addCommand("showDeleteThread",cShowDeleteThread,"showDeleteThread(threadNum);",2,2);
   
   Con::addCommand("showSetFileList",cShowSetFileList,"showSetFileList(path,ext,command);",4,4);

   Con::addCommand("showToggleRoot",cShowToggleRoot,"showToggleRoot();",1,1);
   Con::addCommand("showToggleStick",cShowToggleStick,"showToggleStick();",1,1);
   Con::addCommand("showSetCamera",cShowSetCamera,"showSetCamera(orbitShape);",2,2);
   Con::addCommand("showSetKeyboard",cShowSetKeyboard,"showSetKeyboard(moveShape);",2,2);
   
   Con::addCommand("showSetLightDirection",cShowSetLight,"showSetLightDirection();",1,1);
   Con::addCommand("showSetDetailSlider",cShowSetDetailSlider,"showSetDetailSlider();",1,1);
   ...
   // anything on the command line we should know about...
   // we stake out anything between -show and the next -* parameter
   bool showSomething = false;
   S32 count = Con::getIntVariable("Game::argc");
   for (S32 i=1; i<count; i++)
   {
      const char * param = Con::getVariable(avar("Game::argv%i",i));
      if (showSomething && param && *param=='-')
         break;
         
      if (!dStrcmp(param,"-show"))
      {
         showSomething=true;
         continue;
      }
      ...
      // if we get this far, then the argument is a shape, sequence or mission file name
      // but can't create it yet...just store the names away for later
      if (dStrrchr(param,'\\'))
         param = dStrrchr(param,'\\')+1;
      else if (dStrrchr(param,'/'))
         param = dStrrchr(param,'/')+1;
      ShowTSShape::loadQueue.push_back(dStrdup(param));
      Con::setVariable(avar("Game::argv%i",i),"");
   }

   if (showSomething && ShowTSShape::loadQueue.empty())
   {
      // need at least one thing so we know to start show
      // when the game loop begins...
      ShowTSShape::loadQueue.push_back(dStrdup("skip"));
   }
