...
bool DebrisData::preload(bool server, char errorBuffer[256])
{
   ...
   if( shapeName && shapeName[0] != '\0' && !bool(shape) )
   {
      ...
      shape = ResourceManager->load(fullName);
      if( bool(shape) == false )
      {
...
void DebrisData::initPersistFields()
{
   ...
   Con::registerType(TypeDebrisDataPtr, sizeof(DebrisData*),
   ...
   addField("shapeName",            TypeString,                  Offset(shapeName,           DebrisData));
...
//----------------------------------------------------------------------------
static bool cInitDebris( SimObject *obj, S32, const char **argv )
{
   Debris* debris = static_cast<Debris*>(obj);
...
// Console init (static)
//----------------------------------------------------------------------------
void Debris::consoleInit()
{
   Con::addCommand("Debris", "init", cInitDebris, "obj.init( position, velocity )", 4, 4);
}
...
void Debris::advanceTime( F32 dt )
{
   ...
   updateEmitters( pos, mVelocity, dt * 1000.0 );
