...
class Shadow
{
   ...
   static F32 smGenericRadiusScew;
   static bool smAlwaysUseGenericBmp;
   static F32 smLightDirScew;
   static F32 smLightLenScew;

   static F32 smGlobalShadowDetail;

   static void collisionCallback(SceneObject*,S32);
...
public:
   ...
   {
      F32 dist;
      F32 directionScew; // 0-1, 0 means leave direction alone, 1 means light vector (0,0,-1)
      F32 lengthScew;    // 0-1, 0 means leave length alone, 1 means shadow volume has no depth
   };
