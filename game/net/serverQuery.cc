...
void queryLanServers( U32 port, U8 /*flags*/, bool offline )
{
   ...
   processPingsAndQueries( gPingSession, offline );
}
...
ConsoleFunction( queryMasterServer, void, 12, 12, "queryMasterServer(...);" )
{
   ...
   queryMasterServer(lanPort,flags,rulesSet,missionType,minPlayers,maxPlayers,
...
static void processPingsAndQueries( U32 session, bool schedule )
{
   ...
   U32 time = Platform::getVirtualMilliseconds();
   char addressString[256];
   U8 flags = isClientOnline() ? ServerFilter::OnlineQuery : ServerFilter::OfflineQuery;
...
static void handleGameMasterInfoRequest( const NetAddress* address, U32 key, U8 flags )
{
   if ( gAllowConnections )
   {
      ...
      if ( dStrlen( Con::getVariable( "Pre::Server::Password" ) ) > 0 )
...
static void handleGamePingRequest( const NetAddress* address, U32 key, U8 flags )
{
   ...
   if ( gAllowConnections )
   {
      ...
      if ( isServerOnline() && ( flags & ServerFilter::OfflineQuery ) )
         return;
