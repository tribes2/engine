...
const static U32 sCollisionMoveMask = (TerrainObjectType        | InteriorObjectType   |
                                       ...
                                       VehicleBlockerObjectType | ForceFieldObjectType |
                                       StaticTSObjectType);
...
void FlyingVehicle::updateMove(const Move* move)
{
   ...
   { 
      ...
      if (mRigid.state.linPosition.z > flightCeiling)
      {
         ...
         {
            mCeilingFactor = 1.0f - ((mRigid.state.linPosition.z - flightCeiling) / (flightCeiling + ceilingRange));
...
void FlyingVehicle::updateForces(F32 /*dt*/)
{
   ...
   if (speed < mDataBlock->maxAutoSpeed) {
      ...
      force -= yv * sf * mDot(yv, mRigid.state.linVelocity);
      force -= xv * sf * mDot(xv, mRigid.state.linVelocity);
   }
...
//   }
   ...
   ar -= mDataBlock->rollForce * mDot(xv, mRigid.state.linVelocity);
   torque += yv * ar;
...
////   mRigid.state.linVelocity.z -= mBuoyancy * sFlyingVehicleGravity * mGravityMod * TickSec;
////   mRigid.state.linVelocity -= mRigid.state.linVelocity * mDrag * TickSec;
////   mRigid.state.angVelocity -= mRigid.state.angVelocity * mDrag * TickSec;

   mRigid.state.force  = force  * mOneOverMass;
   mRigid.state.torque = torque * mOneOverMass;
}
...
void FlyingVehicle::updateJet(F32 dt)
{
   ...
   F32 speed = mFabs(mDot(yv,mRigid.state.linVelocity));
   F32 trail = 0;
...
bool FlyingVehicle::writePacketData(GameConnection *connection, BitStream *stream)
{
   return Parent::writePacketData(connection, stream);
}
...
void cUseCreateHeight(SimObject *obj, S32, const char **argv)
{
   FlyingVehicle *ctrl = static_cast<FlyingVehicle*>(obj);
   ctrl->useCreateHeight(dAtob(argv[2]));   
}

void FlyingVehicle::consoleInit()
{
   Con::addCommand("FlyingVehicle", "useCreateHeight", cUseCreateHeight, "FlyingVehicle.setCreateHeight(bool)",  3, 3);
}
