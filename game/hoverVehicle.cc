...
void nonFilter(SceneObject* object,S32 key)
{
...
//--------------------------------------------------------------------------
void HoverVehicle::initPersistFields()
{
   Parent::initPersistFields();
...
F32 HoverVehicle::getBaseStabilizerLength() const
{
   ...
   F32 velLength  = mRigid.state.linVelocity.len();
...
void HoverVehicle::updateForces(F32 /*dt*/)
{
   ...
   mRigid.state.getTransform(&currTransform);
   ...
   Point3F vel = mRigid.state.linVelocity;
   ...
   for (i = 0; i < 2; i++) {
      ...
      stabPoints[i].wsVelocity  = mRigid.state.linVelocity;
   }
   ...
   // Braking
   F32 vellen = mRigid.state.linVelocity.len();
   if (mThrottle == 0.0f &&
       mLeftThrust == 0.0f &&
       mRightThrust == 0.0f &&
       vellen != 0.0f &&
   ...
   {
      Point3F dir = mRigid.state.linVelocity;
      dir.normalize();
      dir.neg();
   ...
   // Gyro Drag 
   torque = -mRigid.state.angMomentum * mDataBlock->gyroDrag;
   ...
   // Apply drag
   Point3F vDrag = mRigid.state.linVelocity;
   if (!mFloating) {
...
//bool HoverVehicle::collideBody(const MatrixF& mat,Collision* info)
//{
...
//   extractor.setTransform(&mat, VectorF(1,1,1));
...
//   }
//
//   info->face = sPolyList->mPolyList.size() ? BodyCollision : 0;
//
...
void HoverVehicle::updateDustTrail( F32 dt )
{
   ...
   if( vel.len() > 2.0  && pEntry)
   {
      ...
      mDustTrailEmitter->emitParticles( contactPoint , true, axis, vel, dt * 1000 * (vel.len() / mDataBlock->dustTrailFreqMod) );
   }
