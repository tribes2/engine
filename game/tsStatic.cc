...
void TSStatic::initPersistFields()
{
   ...
   addField("shapeName", TypeString, Offset(mShapeName, TSStatic));
}
...
bool TSStatic::onAdd()
{
   ...
   char fullName[256];
   dSprintf(fullName,sizeof(fullName),"shapes/%s",mShapeName);
   mShape = ResourceManager->load(fullName);

   if (bool(mShape) == false) {
      AssertFatal(false, avar("TSStatic::onAdd: unable to load shape: %s", mShapeName));
