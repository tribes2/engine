...
void ParticleEmitterData::initPersistFields()
{
   ...
   Con::registerType(TypeParticleEmitterDataPtr, sizeof(ParticleEmitterData*),
...
void ParticleEmitterData::packData(BitStream* stream)
{
   ...
   stream->writeInt(ejectionVelocity * 100, 16);
   stream->writeInt(velocityVariance * 100, 14);
   if(stream->writeFlag(ejectionOffset != gDefaultEmitterData.ejectionOffset))
      stream->writeInt(ejectionOffset * 100, 16);
   stream->writeRangedU32(thetaMin, 0, 180);
   stream->writeRangedU32(thetaMax, 0, 180);
   if(stream->writeFlag(phiReferenceVel != gDefaultEmitterData.phiReferenceVel))
      stream->writeRangedU32(phiReferenceVel, 0, 360);
   if(stream->writeFlag(phiVariance != gDefaultEmitterData.phiVariance))
      stream->writeRangedU32(phiVariance, 0, 360);
...
void ParticleData::initPersistFields()
{
   ...
   addField("textureName",          TypeString, Offset(textureNameList,      ParticleData));
   addField("animTexName",          TypeString, Offset(textureNameList,      ParticleData), PDC_MAX_TEX );
...
void ParticleData::packData(BitStream* stream)
{
   ...
   {
      stream->writeInt(spinRandomMin + 1000, 11);
      stream->writeInt(spinRandomMax + 1000, 11);
   }
