...
static void cActivate(SimObject* obj, S32, const char**)
{
   AssertFatal(dynamic_cast<PhysicalZone*>(obj) != NULL, "Error, how did a non-forcefield get in here?");
   PhysicalZone* pZone = static_cast<PhysicalZone*>(obj);

   if (pZone->isClientObject())
      return;

   pZone->activate();
}

static void cDeactivate(SimObject* obj, S32, const char**)
{
   AssertFatal(dynamic_cast<PhysicalZone*>(obj) != NULL, "Error, how did a non-forcefield get in here?");
   PhysicalZone* pZone = static_cast<PhysicalZone*>(obj);
...
{
   Con::addCommand("PhysicalZone", "activate",   cActivate,   "obj.activate()",   2, 2);
   Con::addCommand("PhysicalZone", "deactivate", cDeactivate, "obj.deactivate()", 2, 2);
}
