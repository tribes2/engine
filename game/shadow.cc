...
F32 Shadow::smLightDirScew = 0.0f;
F32 Shadow::smLightLenScew = 0.0f;
F32 Shadow::smGenericRadiusScew = 0.4f; // shrink radius of shape when it always uses generic shadow...
...
void Shadow::setGlobalShadowDetailLevel(F32 d)
{
   ...
   smLightDirScew = 0.85f * (1.0f-d);
   smLightLenScew = (1.0f-d) * 0.5f;
}
   ...
   for (S32 i=0; i<mNumDistanceDetails; i++)
   {
      if (dist<=mDistanceDetails[i].dist)
      {
         ...
         else
         {
            ...
            dd->directionScew = interp * mDistanceDetails[i-1].directionScew + (1.0f-interp) * mDistanceDetails[i].directionScew;
            dd->lengthScew = interp * mDistanceDetails[i-1].lengthScew + (1.0f-interp) * mDistanceDetails[i].lengthScew;         
         }
         return;
      }
   }
...
{
   AssertFatal(mBitmap,"Shadow::beginRenderToBitmap");
...
bool Shadow::prepare(const Point3F & pos, Point3F lightDir, F32 shadowLen, const Point3F & scale, F32 dist, F32 fogAmount, TSShapeInstance * shapeInstance)
{
   ...
   if (!mSettings.noMove || mPartition.empty())
   {
      // --------------------------------------
      // 1.
      F32 dirMult = (1.0f - dd.directionScew) * (1.0f - smLightDirScew);
      if (dirMult < 0.99f)
      {
         lightDir.z *= dirMult;
...
void Shadow::buildPartition(const Point3F & p, const Point3F & lightDir, F32 radius, F32 shadowLen)
{
   ...
   gClientContainer.findObjects(smShadowMask,Shadow::collisionCallback,S32(this));
...
void Shadow::collisionCallback(SceneObject * obj, S32 thisPtr)
{
