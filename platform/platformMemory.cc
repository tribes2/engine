...
PageRecord *allocPage(U32 pageSize)
{
   ...
   for(PageRecord *list = gPageList; list; list = list->prevPage)
   {
      ...
      for(Header *walk = list->headerList; walk; walk = walk->next)
      {
         ...
         if(walk->prev != prev)
            Platform::debugBreak();
         prev = walk;
         if(walk->next && (U32(walk->next) != U32(walk) + sizeof(Header) + walk->size))
            Platform::debugBreak();
      }
   }
}
...
static FreeHeader *treeFindSmallestGreaterThan(U32 size)
{
...
static Header *allocMemPage(U32 pageSize)
{
...
static void* alloc(U32 size, bool array, const char* fileName, const U32 line)
{
   fileName, line;
...
static void free(void* mem, bool array)
{
   ...
   AssertFatal(bool(hdr->flags & Array) == array, avar("Array alloc mismatch. "));
...
#ifdef DEBUG_GUARD
ConsoleFunction(dumpMemSnapshot,void,2,2,"dumpMemSnapshot(fileName);")
{
   argc;
   FileStream fws;
...
void* dMalloc_r(U32 in_size, const char* fileName, const U32 line)
{
...
void* dRealloc(void* in_pResize, U32 in_size)
{
   return Memory::realloc(in_pResize, in_size);
}

#else // #ifndef NO_MEMORY_MANAGER
