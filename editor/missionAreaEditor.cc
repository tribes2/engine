...
{
   AssertFatal(bool(mMissionArea), "MissionAreaEditor::getArea: no MissionArea obj!");
...
{
   AssertFatal(bool(mMissionArea), "MissionAreaEditor::setArea: no MissionArea obj!");
   if(bool(mMissionArea))
   {
      RectI clamped = area;
...
GuiCursor * MissionAreaEditor::getCursor()
{
   AssertFatal(mCurrentCursor, "MissionAreaEditor::getCursor: no cursor to get!");
...
bool MissionAreaEditor::grabCursors()
{
   struct {
      ...
      {GrabCursor,               "Editor_MoveCursor"  },
      {VertResizeCursor,         "UpDownCursor"       },
      {HorizResizeCursor,        "LeftRightCursor"    },
      {DiagRightResizeCursor,    "DiagRightCursor"    },
      {DiagLeftResizeCursor,     "DiagLeftCursor"     }
   };
...
void MissionAreaEditor::onRender(Point2I offset, const RectI & updateRect, GuiControl * firstResponder)
{
...
void MissionAreaEditor::onMouseDragged(const GuiEvent & event)
{
   ...
   RectI iBox(round_local(min.x), round_local(min.y), round_local(max.x - min.x), round_local(max.y - min.y));
   setArea(iBox);
...
static void cCenterWorld(SimObject * obj, S32, const char **) 
{
   MissionAreaEditor * editor = static_cast<MissionAreaEditor*>(obj);
   if(!editor->missionAreaObjValid())
   {
   ...
   editor->setArea(editor->getArea());
   RectI area = editor->getArea();
   ...
   for(SimSetIterator itr(missionGroup); *itr; ++itr)
   {
      ...
      if(terrain)
      {
         ...
         Point2I start((offset.x / terrain->getSquareSize()) & TerrainBlock::BlockMask, 
                       (offset.y / terrain->getSquareSize()) & TerrainBlock::BlockMask);

         // update the grid block
         if(start.x || start.y)
         {
            U16 heights[TerrainBlock::BlockSize * TerrainBlock::BlockSize];
            U8 baseMaterials[TerrainBlock::BlockSize * TerrainBlock::BlockSize];
            TerrainBlock::Material materials[TerrainBlock::BlockSize * TerrainBlock::BlockSize];
   ...
   Con::executef(editor, 2, "onWorldOffset", buf);
...
static void cSetArea(SimObject * obj, S32 argc, const char ** argv)
{
   MissionAreaEditor * editor = static_cast<MissionAreaEditor*>(obj);
   if(!editor->missionAreaObjValid())
   {
...
static void cUpdateTerrain(SimObject * obj, S32, const char **)
{
   MissionAreaEditor * editor = static_cast<MissionAreaEditor*>(obj);

   //
   if(!editor->getTerrainObj())
   {
...
void MissionAreaEditor::consoleInit()
{
   Con::addCommand("MissionAreaEditor", "centerWorld", cCenterWorld, "missionAreaEditor.centerWorld();", 2, 2);
   Con::addCommand("MissionAreaEditor", "getArea", cGetArea, "missionAreaEditor.getArea();", 2, 2);
   Con::addCommand("MissionAreaEditor", "setArea", cSetArea, "missionAreaEditor.setArea(x, y, w, h);", 3, 6);
   Con::addCommand("MissionAreaEditor", "updateTerrain", cUpdateTerrain, "missionAreaEditor.updateTerrain();", 2, 2);
}
