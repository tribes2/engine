...
//            nextIdx = (j%2)==0 ? &idx0 : &idx1;
            nextIdx = (U32*) ( (U32)nextIdx ^ (U32)&idx0 ^ (U32)&idx1);
...
bool TSMesh::getFeatures(S32 frame, const MatrixF& mat, const Point3F& /*n*/, ConvexFeature* cf, U32& /*surfaceKey*/)
{
      ...
      if ( (draw.matIndex & TSDrawPrimitive::TypeMask) == TSDrawPrimitive::Triangles)
      {
         for (S32 j=0; j<draw.numElements; j+=3)
         {
            ...
            for (U32 l = 0; l < 3; l++) {
               ...
               bool found = false;
               for (U32 k = 0; k < cf->mEdgeList.size(); k++) {
...
bool TSMesh::castRay(S32 frame, const Point3F & start, const Point3F & end, RayInfo * rayInfo)
{
   ...
   for (S32 i=startPlane; i<startPlane+planesPerFrame; i++)
   {
      ...
      {
         sgn *= -1.0f;
         pnum = (F32*) ((U32)pnum ^ (U32)&endNum ^ (U32)&startNum);
         pden = (F32*) ((U32)pden ^ (U32)&endDen ^ (U32)&startDen);
         pplane = (S32*) ((U32)pplane ^ (U32)&tmpPlane ^ (U32)&curPlane);
         pfound = (bool*) ((U32)pfound ^ (U32)&tmpFound ^ (U32)&found);
      }
      bool noCollision;
      if (num * *pden * sgn < *pnum * den * sgn && !(noCollision=num*endDen*sgn<endNum*den*sgn && num*startDen*sgn<startNum*den*sgn))
      {
         *pnum = num;
         *pden = den;
         *pplane = i;
