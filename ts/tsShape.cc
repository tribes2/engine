...
TSShape::~TSShape()
{
   ...
   for (dca = 0; dca < detailCollisionAccelerators.size(); dca++)
   {
      ...
      if (accel != NULL) {
         ...
         for (U32 j = 0; j < accel->numVerts; j++)
   ...
   for (i=0; i<objects.size(); i++)
   {
      ...
      for (S32 dl=0; dl<obj->numMeshes; dl++)
      {
         ...
         if (mesh)
         {
            ...
            maxSize = getMax(maxSize,mesh->mergeIndices.size());
         }
      }
...
void TSShape::setupBillboardDetails()
{
   ...
   if (billboardDetails.empty())
   {
      for (i=0; i<details.size(); i++)
      {
         ...
         F32 polarAngle = 0.5f * M_PI * (1.0f/64.0f) * (F32) ((props>>13) & 0x3F); // bits 13..18
         S32 dl = (props>>19) & 0x0F;  // 19..22
         S32 dim = (props>>23) & 0xFF; // 23..30
         bool includePoles = props & 0x80000000; // bit 31
...
void TSShape::computeBounds(S32 dl, Box3F & bounds) const
{
   ...
   bounds.min.set( 10E30, 10E30, 10E30);
   bounds.max.set(-10E30,-10E30,-10E30);
   Box3F box;
...
void TSShape::fixEndian(S32 * buff32, S16 * buff16, S8 *, S32 count32, S32 count16, S32)
{
   ...
   if (((S16)0x1234)!=convertLEndianToHost((S16)0x1234))
   {
...
void TSShape::readIflMaterials()
{
   ...
   for (S32 i=0; i<iflMaterials.size(); i++)
   {
      ...
      char nameBuffer[128];
      if (mReadVersion>=0 && mReadVersion<16)
         // older file format...file name doesn't include prefix
         dSprintf(nameBuffer,sizeof(nameBuffer),"%s%s", TSMaterialList::csmIFLTexturePrefix, 
                                                          getName(iflMaterial.nameIndex));
      else
      {
         // bad hack -- see below for similar case...
         if (dStrcmp(TSMaterialList::csmIFLTexturePrefix,TSMaterialList::csmTSTexturePrefix))
            // newer file format...file name does include prefix
            dSprintf(nameBuffer,sizeof(nameBuffer),"%s", getName(iflMaterial.nameIndex));
         else
            // newer file format...but file name doesn't include prefix (hack for hunting...this'll be cleaned up later)
            dSprintf(nameBuffer,sizeof(nameBuffer),"%s%s", TSMaterialList::csmIFLTexturePrefix, getName(iflMaterial.nameIndex));
      }
      ...
      while (s->getStatus() == Stream::Ok)
      {
         ...
         if (s->getStatus() == Stream::Ok || s->getStatus() == Stream::EOS)
         {
            ...
            pos = buffer;      
            // strip off any preceding spaces
            while (*pos && *pos==' ')
               pos++;
            ...
            if (pos2)
            {
               *pos2='\0';
               pos2++;
               // strip off preceding spaces from duration
               while (*pos2 && *pos2==' ')
                  pos2++;
            }
            duration = pos2 ? dAtoi(pos2) : 1;
            if (duration==0)
               duration = 1; // just in case...
            if (!*pos)
               // skip line with no material
               continue;
            dStrncpy(name,pos,256);

            totalTime += duration;
            iflFrameOffTimes.push_back((1.0f/30.0f) * (F32)totalTime); // ifl units are frames (1 frame = 1/30 sec)

            // bad hack -- but should work
            // problem is that ifl texture prefix is needed to get ifl file but individual materials
            // in the ifl may or may not need prefix...the following fix works for Tribes II and Hunting
            // At some point in the future, ifl's should be handled better (probably by incorporating
            // them into the shape...)
            if (!dStrcmp(TSMaterialList::csmIFLTexturePrefix,TSMaterialList::csmTSTexturePrefix))
               dSprintf((char*)buffer,sizeof(buffer),"%s",name);
            else
               dSprintf((char*)buffer, sizeof(buffer), "%s%s", TSMaterialList::csmIFLTexturePrefix, name);
            materialList->push_back((char*)buffer,TSMaterialList::IflFrame);
...
void TSShape::computeAccelerator(S32 dl)
{
   ...
   for (i = 0; i < cf.mVertexList.size(); i++) {
      ...
      if (found == true) {
         ...
         for (U32 k = 0; k < cf.mFaceList.size(); k++) {
            for (U32 l = 0; l < 3; l++) {
   ...
   for (i = 0; i < accel->numVerts; i++) {
      ...
      for (j = 0; j < faces.size(); j++) {
         for (U32 k = 0; k < 3; k++) {
            ...
            bool found = false;
            for (U32 l = 0; l < vertRemaps.size(); l++) {
