...
void TSThread::selectKeyframes(F32 pos, const TSSequence * seq, S32 * k1, S32 * k2, F32 * kpos)
{
   ...
   else
   {
      ...
      else
      {
         ...
         // following assert could happen if pos1<1 && pos1==1...paradoxically...
         AssertFatal(kfIdx1<seq->numKeyframes,"TSThread::advancePos");
...
{
   const TSShape * shape = mShapeInstance->mShape;
   AssertFatal(shape && shape->sequences.size()>seq && toPos>=0.0f && toPos<=1.0f,"TSThread::setSequence: invalid");
...
void TSThread::activateTriggers(F32 a, F32 b)
{
   ...
   if (aIndex<=bIndex)
   {
      for (i=aIndex; i<bIndex; i++)
      {
         ...
         bool on = state & TSShape::Trigger::StateOn;
...
{
   AssertFatal(threadNumber < mThreadList.size() && threadNumber>=0,"TSShapeInstance::getThread");
...
void TSShapeInstance::destroyThread(TSThread * thread)
{
   ...
   AssertFatal(i<mThreadList.size(),"TSShapeInstance::destroyThread");
...
S32 TSShapeInstance::threadCount()
{
...
S32 TSShapeInstance::getSequence(TSThread * thread)
{
   AssertFatal( (S32)(thread->sequence - mShape->sequences.address())>=0,
      "TSShapeInstance::getSequence: range error A");
   AssertFatal( (S32)(thread->sequence - mShape->sequences.address())<mShape->sequences.size(),
      "TSShapeInstance::getSequence: range error B");
   return (S32) (thread->sequence - mShape->sequences.address());
}
