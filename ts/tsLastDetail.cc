                           ...
                           S32 dl, S32 dim)
{
   ...
   F32 equatorStepSize = M_2PI / (F32) numEquatorSteps;
   F32 polarStepSize = numPolarSteps>0 ? (0.5f * M_PI - polarAngle) / (F32)numPolarSteps : 0.0f;

   S32 i;
   F32 rotZ = 0;
   for (i=0; i<numEquatorSteps; i++)
   {
      F32 rotX = numPolarSteps>0 ? polarAngle - 0.5f * M_PI : 0.0f;
      for (S32 j=0; j<2*numPolarSteps+1; j++)
      {
         MatrixF angMat;
         angMat.mul(MatrixF(EulerF(0,0,-M_PI+rotZ)),MatrixF(EulerF(rotX,0,0)));
   ...
   if (includePoles)
   {
      MatrixF m1( EulerF( M_PI / 2, 0, 0 ) );
      MatrixF m2( EulerF( -M_PI / 2, 0, 0 ) );
...
//      mRotY = mAtan(y.y,x.y);
   ...
   {
      F32 equatorStepSize = M_2PI / (F32) mNumEquatorSteps;
      F32 polarStepSize = mNumPolarSteps>0 ? (0.5f * M_PI - mPolarAngle) / (F32) mNumPolarSteps : 0.0f;
      rotZ = 0.999f * (mAtan(dotX,dotY) + M_PI); // the 0.99f makes sure we are in range
      AssertFatal(rotZ>=0 && rotZ<M_2PI,"TSLastDetail::chooseView: rotZ out of range");
