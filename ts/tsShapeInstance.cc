...
void tsShapeTextureEventCB(const U32 eventCode, const U32 userData)
{
      ...
      pShape->mVertexBuffer != -1)
   {
      ...
      pShape->mVertexBuffer = -1;
      for (U32 i = 0; i < pShape->objects.size(); ++i)
...
void TSShapeInstance::setMaterialList(TSMaterialList * ml)
{
   ...
   if (mMaterialList && StringTable) // material lists need the string table to load...
   {
      ...
      if (mShape->materialList == mMaterialList)
         ((TSShape*)mShape)->readIflMaterials();

      // Transform the names in the material list if necessary
      mMaterialList->tsmlTransform();
      mMaterialList->load(MeshTexture,true);

      // check for reflectance map not in alpha of texture -- will require more work to emap
      for (S32 i=0; i<mMaterialList->getMaterialCount(); i++)
      {
...
bool TSShapeInstance::renderMeshesX(S32 ss, S32 od)
{
   ...
   if ((vb = mShape->mVertexBuffer) == -1)
   {
      ...
      vb = mShape->mVertexBuffer = glAllocateVertexBufferEXT(size,GL_TRIBESMTNVFMT_EXT,true);
      if (vb == -1)
      {
         PROFILE_END();
         return false;
      ...
      if (mShape->mCallbackKey == -1)
         mShape->mCallbackKey = TextureManager::registerEventCallback(tsShapeTextureEventCB, U32(mShape));
...
void TSShapeInstance::setupFog(F32 fogAmount, const ColorF & fogColor)
{
       ...
       smRenderData.fogColor.z != fogColor.blue)
   {
      ...
      U8 red = 255*fogColor.red;
      U8 green = 255*fogColor.green;
      U8 blue = 255*fogColor.blue;
...
{
   AssertFatal((TextureObject*)mEnvironmentMap,"TSShapeInstance::renderEnvironmentMap (1)");
...
S32 TSShapeInstance::selectCurrentDetail2(F32 adjustedDist)
{
   ...
   F32 pixelRadius = dglProjectRadius(adjustedDist,mShape->radius) * dglGetPixelScale() * smDetailAdjust;
   return selectCurrentDetail(pixelRadius);
}
...
S32 TSShapeInstance::selectCurrentDetail(F32 size)
{
        ...
        (mCurrentDetailLevel>0  && (size<=mShape->details[mCurrentDetailLevel].size || size>mShape->details[mCurrentDetailLevel-1].size)))
   {
      ...
      for (S32 i=0; i<mShape->details.size(); i++)
      {
         ...
         AssertFatal(i+1<mShape->details.size() && mShape->details[i+1].size>=0,"TSShapeInstance::selectCurrentDetail");
      }
   }
...
{
   S32 screenWidth = Platform::getWindowSize().x;
   S32 screenHeight = Platform::getWindowSize().y;
   S32 xcenter = screenWidth >> 1;
   S32 ycenter = screenHeight >> 1;
         ...
         for (j=0; j<width; j++)
         {
            ...
            for (S32 k=0; k<scale; k++)
            {
               for (S32 l=0; l<scale; l++)
               {
                  ...
                  // we want 255 * alhpa, or 255 - (white - black)
                  U32 pos = 3*((i*scale+k)*bmpWidth+j*scale+l);
                  S32 alpha = 255 - (wbmp[pos+0] - bbmp[pos+0]);
   ...
   else
   {
      ...
      for (i=0; i<height*width; i++)
      {
         // shape on black background is alpha * color, shape on white background is alpha * color + (1-alpha) * 255
         // we want 255 * alhpa, or 255 - (white - black)
         S32 alpha = 255 - (wbmp[i*3+0] - bbmp[i*3+0]);
...
void TSShapeInstance::MeshObjectInstance::morphVB(S32 vb, S32 &previousMerge, S32 objectDetail, TSMaterialList *materials)
{
   if (visible > 0.01f)
   {
      ...
      if (m0 && mesh)
      {
         ...
         U32 merge = mesh->vertsPerFrame-morphSize;
