...
{
   S32 tmp;
   s->read(sizeof(tmp),(U8*)&tmp);
   U32 * ptr = (U32*)oldAlloc(memBuffer32,1);
   *ptr = tmp;
   return (S32)convertLEndianToHost(tmp);
}
...
void oldAllocGuard(S32 * & addr32, S16 * & addr16, S8 * & addr8)
{
   ...
   *ptr32 = convertLEndianToHost(OldAlloc::guard32++);

   S16 * ptr16 = (S16*)oldAlloc(addr16,1);
   *ptr16 = convertLEndianToHost(OldAlloc::guard16++);
...
void readAllocMesh(Stream * s, S32 * & memBuffer32, S16 * & memBuffer16, S8 * & memBuffer8, U32 meshType)
{
   ...
   *ptr32 = convertLEndianToHost(-1);
   ...
   if (TSShape::smReadVersion<18)
   {
      for (S32 j=0; j<sz; j++)
      {
         ...
         ptr16[0]=convertLEndianToHost((S16)a);
         ptr16[1]=convertLEndianToHost((S16)b);
   ...
   *ptr32 = convertLEndianToHost((S32)0);
   ...
   if (meshType==TSMesh::SortedMeshType)
   {
      ...
      s->read(&alwaysWriteZ);
      S32 * ptr32 = (S32*) oldAlloc(memBuffer32,1);
      *ptr32 = alwaysWriteZ ? convertLEndianToHost(1) : convertLEndianToHost(0);
                           ...
                           S32 & count32, S32 & count16, S32 & count8)
{
   ...
   for (i=0; i<3; i++)
   {
      S32 prev = i==0 ? numNodes : (i==1 ? numObjects : numDecals);
      for (S32 j=numSubShapes-1; j>=0; j--)
      {
         pSubShapeNumStart[j] = prev - convertLEndianToHost(pSubShapeFirstStart[j]);
         prev = convertLEndianToHost(pSubShapeFirstStart[j]);
      }
   ...
   {
      S32 dsize = convertLEndianToHost(pd[i].size);
      if (dsize>=0.0f)
      {
   ...
   if (gotList)
   {
      ...
      if (mExporterVersion<116)
      {
         ...
         for (i=0; i<materialList->getMaterialCount(); i++)
         {
...
{
#ifndef MAX_UTIL // this method not necessary in exporter, and a couple lines won't compile for exporter
