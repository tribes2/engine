...
bool DepthSortList::splitPoly(const Poly & src, Point3F & normal, F32 k, Poly & frontPoly, Poly & backPoly)
{
   ...
   // already degenerate...
   AssertFatal(src.vertexCount>=3,"DepthSortList::splitPoly");
...
Vector<DepthSortList::Poly> gWorkListA;
Vector<DepthSortList::Poly> gWorkListB;
...
void DepthSortList::depthPartition(const Point3F * sourceVerts, U32 numVerts, Vector<Poly> & partition, Vector<Point3F> & partitionVerts)
{
   ...
   // a depth partition is a parition of the poly on the
   // x-z plane s.t. each sub-poly in the partition can be
   ...
   while (sourceList->size())
   {
      if (i>=mBase && !sortNext())
         ...
         break;
      AssertFatal(i<=mBase,"DepthSortList::depthPartition");
      ...
      AssertFatal(mFabs(cutter.plane.y)>=MIN_Y_DOT,"DepthSortList::depthPartition");
