...
//-------------------------------------- Console callbacks
//
const char* cGetTransform(SimObject *obj, S32, const char **)
{
   char *returnBuffer = Con::getReturnBuffer(256);
   SceneObject *ptr = (SceneObject *) obj;
   const MatrixF& mat = ptr->getTransform();
   Point3F pos;
   mat.getColumn(3,&pos);
   AngAxisF aa(mat);
...
const char *cGetForwardVector(SimObject *obj, S32, const char **)
{
   char *returnBuffer = Con::getReturnBuffer(256);
   SceneObject *ptr = (SceneObject *) obj;
   const MatrixF& mat = ptr->getTransform();
...
void cSetTransform(SimObject *obj, S32, const char **argv)
{
   SceneObject *ptr = (SceneObject *) obj;
   Point3F pos;
   const MatrixF& tmat = ptr->getTransform();
...
const char* cGetScale(SimObject * obj, S32, const char **)
{
   char *returnBuffer = Con::getReturnBuffer(256);
   SceneObject * sceneObj = static_cast<SceneObject*>(obj);
   const VectorF & scale = sceneObj->getScale();
   dSprintf(returnBuffer, 256, "%g %g %g",
            scale.x, scale.y, scale.z);
   return(returnBuffer);
}

void cSetScale(SimObject * obj, S32, const char ** argv)
{
   SceneObject * sceneObj = static_cast<SceneObject*>(obj);
...
const char* cGetWorldBox(SimObject *obj, S32, const char**)
{
   char *returnBuffer = Con::getReturnBuffer(256);
   SceneObject *ptr = (SceneObject *) obj;
   const Box3F& box = ptr->getWorldBox();
...
const char* cGetWorldBoxCenter(SimObject *obj, S32, const char**)
{
   char *returnBuffer = Con::getReturnBuffer(256);
   SceneObject *ptr = (SceneObject *) obj;
   const Box3F& box = ptr->getWorldBox();
...
const char* cGetObjectBox(SimObject *obj, S32, const char**)
{
   char *returnBuffer = Con::getReturnBuffer(256);
   SceneObject *ptr = (SceneObject *) obj;
   const Box3F& box = ptr->getObjBox();
   dSprintf(returnBuffer,256,"%g %g %g %g %g %g",
            box.min.x, box.min.y, box.min.z,
            box.max.x, box.max.y, box.max.z);
   return returnBuffer;
...
// See if any objects of given types are present in box of given extent.  Extent parameter is
// last since only one radius is often needed.  Can change if convention clash annoys anyone...
static bool cContainerBoxEmpty(SimObject*, S32 argc, const char * * argv)
{
   Point3F  center;
   Point3F  extent;
...
void cInitContainerRadiusSearch(SimObject*, S32, const char** argv)
{
   F32 x, y, z;
...
S32 cContainerSearchNext(SimObject*, S32, const char**)
{
   return gServerContainer.containerSearchNext();
}


F32 cContainerSearchCurrDist(SimObject*, S32, const char**)
{
   return gServerContainer.containerSearchCurrDist();
}

F32 cContainerSearchCurrRadDamageDist(SimObject*, S32, const char**)
{
   return gServerContainer.containerSearchCurrRadDamageDist();
}
...
void SceneObject::consoleInit()
{
   Con::addCommand("SceneObject", "getPosition",       cGetPosition,       "obj.getPosition()",      2, 2);
   Con::addCommand("SceneObject", "setTransform",      cSetTransform,      "obj.setTransform(T)",     3, 3);
   Con::addCommand("SceneObject", "getTransform",      cGetTransform,      "obj.getTransform()",      2, 2);
   Con::addCommand("SceneObject", "setScale",          cSetScale,          "obj.setScale(<xs ys zs>)",3, 3);
   Con::addCommand("SceneObject", "getScale",          cGetScale,          "obj.getScale()",          2, 2);
   Con::addCommand("SceneObject", "getWorldBox",       cGetWorldBox,       "obj.getWorldBox()",       2, 2);
   Con::addCommand("SceneObject", "getWorldBoxCenter", cGetWorldBoxCenter, "obj.getWorldBoxCenter()", 2, 2);
   Con::addCommand("SceneObject", "getObjectBox",      cGetObjectBox,      "obj.getObjectBox()",      2, 2);
   Con::addCommand("SceneObject", "getForwardVector",  cGetForwardVector,  "obj.getForwardVector()",  2, 2);

   // Functions for searching the container database.  Note that these will ONLY work
   //  on the server...
   Con::addCommand("InitContainerRadiusSearch",        cInitContainerRadiusSearch,        "InitContainerRadiusSearch(\"x y z\", radius, mask)", 4, 4);
   Con::addCommand("ContainerSearchNext",              cContainerSearchNext,              "ContainerSearchNext()", 1, 1);
   Con::addCommand("ContainerSearchCurrDist",          cContainerSearchCurrDist,          "ContainerSearchCurrDist()", 1, 1);
   Con::addCommand("ContainerSearchCurrRadDamageDist", cContainerSearchCurrRadDamageDist, "ContainerSearchCurrRadDamageDist()", 1, 1);

   Con::addCommand("ContainerRayCast", cContainerRayCast, "ContainerRayCast(\"x y z\", \"x y z\", mask, [exempt object])", 4, 5);
   Con::addCommand("ContainerBoxEmpty", cContainerBoxEmpty, "ContainerBoxEmpty(Mask, Loc, Rad [,yRad, zRad]);", 4, 6);
}
...
void Container::findObjects(const Box3F& box, U32 mask, FindCallback callback, S32 key)
{
   U32 minX, maxX, minY, maxY;
...
void Container::polyhedronFindObjects(const Polyhedron& polyhedron, U32 mask, FindCallback callback, S32 key)
{
   U32 i;
   Box3F box;
...
static void buildCallback(SceneObject* object,S32 key)
{
...
bool Container::buildPolyList(const Box3F& box, U32 mask, AbstractPolyList* polyList,FindCallback callback,S32 key)
{
   CallbackInfo info;
   ...
   sPolyList = polyList;
   findObjects(box,mask,callback? callback: buildCallback,S32(&info));
                                   ...
                                   const Box3F*   queryExpansion)
{
   ...
   // Query main database
   findObjects(queryBox,mask,callback? callback: buildCallback,S32(&info));
                                 ...
                                 const U32      searchMask)
{
   ...
   findObjects(queryBox, searchMask, SimpleQueryList::insertionCallback, S32(&queryList));
...
F32 Container::containerSearchCurrRadDamageDist()
{
   AssertFatal(this == &gServerContainer, "Abort.  Searches only allowed on server container");
   AssertFatal(mCurrSearchPos != -1, "Error, must call containerSearchNext before containerSearchCurrDist");
...
//----------------------------------------------------------------------------
void SimpleQueryList::insertionCallback(SceneObject* obj, S32 key)
{
...
void SceneObject::installLights()
{
   ...
   AssertFatal(lightManager, "SceneObject::installLights: LightManager not found");
...
{
   LightManager * lightManager = gClientSceneGraph->getLightManager();
   AssertFatal(lightManager, "SceneObject::uninstallLights: LightManager not found");
...
{
   AssertFatal(col, "SceneObject::getLightingAmbientColor: invalid color ptr");
   ...
   if(mLightingInfo.mDirty)
   {
      ...
      // look up/down:
      RayInfo collision;
      if(gClientContainer.castRay(pos, Point3F(pos.x, pos.y, pos.z + cRayLength), InteriorObjectType, &collision))
      {
         if(gClientContainer.castRay(pos, Point3F(pos.x, pos.y, pos.z - cRayLength), InteriorObjectType, &collision))
         {
            // interior:
            InteriorInstance * instance = dynamic_cast<InteriorInstance*>(collision.object);

            if((instance && (collision.face != -1)))
            {  
               // determine the color of this point...
               Interior * interior = instance->getDetailLevel(0);
               AssertFatal(interior, "SceneObject::getLightingAmbientColor: invalid interior");
      ...
      else if(gClientContainer.castRay(pos, Point3F(pos.x, pos.y, pos.z - cTerrainRayLength), TerrainObjectType, &collision))
      {
         ...
         if(terrain)
         {
            ...
            AssertFatal(lightmap, "SceneObject::getAmbientLightingColor: no lightmap on terrain");
