...
void ActionMap::dumpActionMap(const char* fileName, const bool append) const
{
   if (fileName != NULL) {
      ...
      FileStream iostrm;
      if ( !ResourceManager->openFileForWrite( iostrm, ResourceManager->getBasePath(), fileName, append ? FileStream::WriteAppend : FileStream::Write ) )
      {
      ...
      else
      {
         ...
         dStrcpy( lineBuffer, "// Tribes 2 Input Map File\n" );
         ...
         for (S32 j = 0; j < pDevMap->nodeMap.size(); j++) {
            ...
            if (rNode.flags & Node::BindCmd) {
               if (rNode.makeConsoleCommand) {
                  dStrcat(lineBuffer, ", \"");
                  dStrcat(lineBuffer, rNode.makeConsoleCommand);
                  dStrcat(lineBuffer, "\"");
               } else {
                  dStrcat(lineBuffer, ", \"\"");
               }
               if (rNode.breakConsoleCommand) {
                  dStrcat(lineBuffer, ", \"");
                  dStrcat(lineBuffer, rNode.breakConsoleCommand);
                  dStrcat(lineBuffer, "\"");
               } 
               else
                  dStrcat(lineBuffer, ", \"\"");
...
bool ActionMap::createEventDescriptor(const char* pEventString, EventDescriptor* pDescriptor)
{
   ...
   //
   AssertFatal(dStrlen(pObjectString) != 0, "Error, no key?");
...
const char* ActionMap::getModifierString(const U32 modifiers)
{
   ...
   switch (realModifiers & (SI_SHIFT|SI_CTRL|SI_ALT)) {
...
{
   static_cast<ActionMap*>( object )->processBind( argc - 2, argv + 2 );
}
...
{
   argc;
   static_cast<ActionMap*>( object )->processBindCmd( argv[2], argv[3], argv[4], argv[5] );
}
...
{
   argc;
   static_cast<ActionMap*>( object )->processUnbind( argv[2], argv[3] );
}

//------------------------------------------------------------------------------
ConsoleMethod( ActionMap, save, void, 2, 4, "actionMap.save( [fileName], [append] )" )
{
   AssertFatal( dynamic_cast<ActionMap*>( object ), "Error, how did a non-actionmap get here?" );
   ActionMap* pMap = static_cast<ActionMap*>( object );

   const char* fileName = argc > 2 ? argv[2]        : NULL;
   bool append          = argc > 3 ? dAtob(argv[3]) : false;

   pMap->dumpActionMap( fileName, append );
}
...
ConsoleMethod( ActionMap, pop, void, 2, 2, "actionMap.pop()" )
{
   ...
   ActionMap *pMap = static_cast<ActionMap*>( object );
   SimSet* pActionMapSet = Sim::getActiveActionMapSet();
   pActionMapSet->removeObject( pMap );
}
...
{
   argc;
	return( static_cast<ActionMap*>( object )->getBinding( argv[2] ) );	
}
...
{
   argc;
	return( static_cast<ActionMap*>( object )->getCommand( argv[2], argv[3] ) );	
}
...
{
   argc;
	return( static_cast<ActionMap*>( object )->isInverted( argv[2], argv[3] ) );
}
...
{
   argc;
	return( static_cast<ActionMap*>( object )->getScale( argv[2], argv[3] ) );
}
...
{
   argc;
	return( static_cast<ActionMap*>( object )->getDeadZone( argv[2], argv[3] ) );
}
