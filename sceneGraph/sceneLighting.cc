...
class SceneLightingProcessEvent : public SimEvent
{
   ...
   public:
      ...
      {
         AssertFatal(obj, "SceneLightingProcessEvent:: null event object!");
         if(obj)
            static_cast<SceneLighting*>(obj)->processEvent(mLightIndex, mObjectIndex);
      };
};
...
//------------------------------------------------------------------------------
U32 SceneLighting::PersistInfo::smFileVersion   = 0x0f;
...
bool SceneLighting::loadPersistInfo(const char * fileName)
{
   ...
   // the scene objects may not be in the same order as the chunks: O(n^2)
   U32 i;
   for(i = 0; i < mSceneObjects.size(); i++)
   {
      bool found = false;
      for(U32 j = 1; j < persistInfo.mChunks.size() && !found; j++)
         if(mSceneObjects[i]->isValidChunk(persistInfo.mChunks[j]))
         {
            found = true;
            chunks.push_back(persistInfo.mChunks[j]);
         }
...
bool SceneLighting::savePersistInfo(const char * fileName)
{
   // open the file
   FileStream file;
   if(!ResourceManager->openFileForWrite(file, ResourceManager->getModPathOf(fileName), fileName))
      return(false);
...
bool SceneLighting::light(BitSet32 flags)
{
   ...
   gClientContainer.findObjects(InteriorObjectType | TerrainObjectType, findObjectsCallback, (S32)&objects);
   ...
   dSprintf(mFileName, sizeof(mFileName), "lighting/%s_%x.ml", Con::getVariable("$MissionName"), missionCRC);
   ...
   if(!flags.test(ForceAlways|ForceWritable))
   {
      if(loadPersistInfo(mFileName))
      {
         ...
         // touch this file...
         char fileBuf[1024];
         dSprintf(fileBuf, sizeof(fileBuf), "base/%s", mFileName);

         if(!dFileTouch(fileBuf))
            Con::warnf("  Failed to touch file '%s'.  File may be read only.", fileBuf);
   ...
   if(!flags.test(ForceAlways))
   {
      ...
      if(!ResourceManager->openFileForWrite(fileStream, ResourceManager->getModPathOf(mFileName), mFileName))
      {
      ...
      if(light == mLights.size())
      {
         ...
         if(Con::getBoolVariable("$pref::sceneLighting::cacheLighting", true))
         {
            ...
            else
            {
               ...
               // update the resource manager (need size of this file now)
               U32 size = dStrlen(ResourceManager->getModPaths());
               if(size)
               {
                  char * pathBuffer = new char [size+1];
                  dMemcpy(pathBuffer, ResourceManager->getModPaths(), size+1);

                  Vector<char*> paths;
                  char * token = dStrtok(pathBuffer, ";");
                  ...
                  ResourceManager->setModPaths(paths.size(), const_cast<const char**>(paths.address()));
                  delete [] pathBuffer;
               }
            }
         }
...
void SceneLighting::processCache()
{
   ...
   S32 curCacheSize = 0;
   match = ResourceManager->findMatch("lighting/*.ml", &name, 0);
   while(match)
   {
      if(match->flags & ResourceObject::File)
      {
         ...
         if(!dStrstr(name, mFileName))
         {
            ...
            // get out of vfs...
            char fileName[1024];
            dSprintf(fileName, sizeof(fileName), "%s/%s", match->filePath, match->fileName);
      ...
      match = ResourceManager->findMatch("lighting/*.ml", &name, match);
   }
   ...
   while(((curCacheSize >> 10) > quota) && files.size())
   {
      ...
      ResourceManager->purge(files.last().mFileObject);
   ...
   for(U32 i = 0; i < interior->getResource()->getNumDetailLevels(); i++)
   {
      ...
      for(U32 j = 0; j < shadowVolume.mSurfaces.size(); j++)
      {
         ...
         pStart[axis] = ((pNormal[si] * pStart[si]) +
                         (pNormal[ti] * pStart[ti]) + plane.d) / -pNormal[axis];
         ...
         // scale the vectors
         F32 lumelScale = lGenX[si] * normLightmap->getWidth();
...
void SceneLighting::TerrainProxy::lightVector(LightInfo * light)
{
   ...
   for(x = 0; x < generateDim; x++)
   {
      ...
      for(y = 0; y < generateDim; y++)
      {
         ...
         if(!lxmask && !lymask && mShadowMask.test((x >> blockShift) + ((y >> blockShift) << TerrainBlock::BlockShift)))
         {
            ...
            if(colorScale > 0.f)
            {
               ...
               {
                  Point2F wPos((x >> lightmapShift) * lexelDim + worldOffset.x,
                               (y >> lightmapShift) * lexelDim + worldOffset.y);
