...
{
   for (S32 i=0; i<mDetailData.size(); i++)
...
void DetailManager::begin()
{
   ...
   mPolyLimit = smMinPolyLimit + smDetailScale * (smMaxPolyLimit-smMinPolyLimit);
...
void DetailManager::computePriority(DetailData * detailData, S32 bump)
{
   ...
   if (detailData->shapeInstance)
   {
      // shape instance...
      if (bump>0)
         oldSize = detailData->dl>=0 ? detailData->shapeInstance->getShape()->details[detailData->dl].size : 0;
      else
         oldSize = detailData->pixelSize;
      newSize = detailData->dls[bump]>=0 ? detailData->shapeInstance->getShape()->details[detailData->dls[bump]].size : 0;
   }
   else
   {
      // part instance...
      ...
      if (bump>0)
         oldSize = detailData->partInstance->getDetailSize(detailData->dl);
      else
         oldSize = detailData->pixelSize;
      newSize = detailData->partInstance->getDetailSize(detailData->dls[bump]);
   }
