...
void SceneGraph::buildFogTextureSpecial(SceneState *pState)
{
   ...
   if(block)
   {
      ...
      if(mHazeArrayDirty)
      {
         ...
         {
            mHazeArray[i] = pState->getHaze(dist);
            mHazeArrayi[i] = mHazeArray[i] * 255;
...
void SceneGraph::traverseSceneTree(SceneState* pState)
{
   ...
   if (pState->mParent != NULL) {
      ...
      AssertFatal(pPortalOwner != NULL && portalIndex != 0xFFFFFFFF, "Hm, this should never happen.  We should always have an owner and an index here");
...
void SceneGraph::registerZones(SceneObject* obj, U32 numZones)
{
   ...
   pQueryContainer->findObjects(obj->mWorldBox, 0xFFFFFFFF, SimpleQueryList::insertionCallback, S32(&list));
