...
void PotentialRenderList::setupClipPlanes(SceneState* state)
{
   ...
   viewPlanes[0].set(camPos,       farPosLeftUp,    farPosLeftDown);
   viewPlanes[1].set(camPos,       farPosRightUp,   farPosLeftUp);
   viewPlanes[2].set(camPos,       farPosRightDown, farPosRightUp);
   viewPlanes[3].set(camPos,       farPosLeftDown,  farPosRightDown);
...
void prlInsertionCallback(SceneObject* obj, S32 key)
{
                                ...
                                const U32    objectMask )
{
      ...
      if (pTraversalRoot->prepRenderImage(state, smStateKey, rootZone, true)) {
         if (pTraversalRoot->getNumCurrZones() != 1)
            Con::errorf(ConsoleLogEntry::General,
                        "Error, must have one and one one zone to be a traversal root.  %s has %d",
   ...
   gClientContainer.findObjects(prl.mBox, objectMask, prlInsertionCallback, S32(&prl));

   // Clear the object colors
   U32 i;
   for (i = 0; i < prl.mList.size(); i++)
      prl.mList[i]->setTraverseColor(SceneObject::White);

   for (i = 0; i < prl.mList.size(); i++)
      if (prl.mList[i]->getTraverseColor() == SceneObject::White)
                                   ...
                                   const U32    stateKey)
{
   ...
   AssertFatal(obj->getTraverseColor() == SceneObject::White,
               "Wrong color for this stage of the traversal!");
   obj->setTraverseColor(SceneObject::Grey);
   ...
   while (pWalk) {
      ...
      if (pOwner->getTraverseColor() == SceneObject::White)
