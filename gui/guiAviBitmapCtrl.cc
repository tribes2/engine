...
//----------------------------------------------------------------------------
static void cSetFilename(SimObject *obj, S32, const char **argv)
{
   GuiAviBitmapCtrl* movie = static_cast<GuiAviBitmapCtrl*>(obj);

   movie->setFilename(argv[2]);
}

//----------------------------------------------------------------------------
static void cPlay(SimObject *obj, S32, const char **)
{
   GuiAviBitmapCtrl* movie = static_cast<GuiAviBitmapCtrl*>(obj);
...
//----------------------------------------------------------------------------
static void cStop(SimObject *obj, S32, const char **)
{
   GuiAviBitmapCtrl* movie = static_cast<GuiAviBitmapCtrl*>(obj);
...
S32 GuiAviBitmapCtrl::fileOpen()
{
   ...
   dSprintf(fileBuffer, sizeof(fileBuffer), "base/textures/%s", mAviFilename);
   rval = AVIFileOpen(&mPFile, fileBuffer, OF_SHARE_DENY_WRITE, 0);
   if (rval)
   {
      fileClose();
      return rval;
...
//----------------------------------------------------------------------------
void GuiAviBitmapCtrl::consoleInit()
{
    Con::addCommand("GuiAviBitmapCtrl", "setFilename", cSetFilename, "obj.setFilename(filename);", 3, 3);
    Con::addCommand("GuiAviBitmapCtrl", "play", cPlay, "obj.play();", 2, 2);
    Con::addCommand("GuiAviBitmapCtrl", "stop", cStop, "obj.stop();", 2, 2);
}
...
//----------------------------------------------------------------------------
void GuiAviBitmapCtrl::onRender(Point2I offset, const RectI &updateRect, GuiControl *firstResponder)
{
   vidsVideoDraw();
...
bool GuiAviBitmapCtrl::onWake()
{
   ...
   AssertFatal(mSurface, "Out of memory");

   // Target the decoding to our surface
   mDecodeLock = SDL_CreateMutex();
   AssertFatal(mDecodeLock, "Out of memory");
