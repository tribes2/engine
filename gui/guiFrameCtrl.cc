...
/******************************************************************************
 *  FILENAME:       D:\Tribes\darkstar\GUI\guiFrameCtrl.cc
 *
 *  DESCRIPTION:    
 *
 *  CREATED:        10/11/99  10:01:16
 *
 *  BY:             PeteW
 ******************************************************************************/
...
//-----------------------------------------------------------------------------
static void cGFSCFrameBorderEnable(SimObject *obj, S32 argc, const char **argv)
{
   GuiFrameSetCtrl *gfsc = dynamic_cast<GuiFrameSetCtrl *>(obj);
   if (gfsc != NULL)
   {
      S32 index = dAtoi(argv[2]);
      if (argc == 3)
         gfsc->frameBorderEnable(index);
      else
         gfsc->frameBorderEnable(index, argv[3]);
   }
}

static void cGFSCFrameBorderMovable(SimObject *obj, S32 argc, const char **argv)
{
   GuiFrameSetCtrl *gfsc = dynamic_cast<GuiFrameSetCtrl *>(obj);
   if (gfsc != NULL)
   {
      S32 index = dAtoi(argv[2]);
      if (argc == 3)
         gfsc->frameBorderMovable(index);
      else
         gfsc->frameBorderMovable(index, argv[3]);
   }
}

static void cGFSCFrameMinExtent(SimObject *obj, S32, const char **argv)
{
   GuiFrameSetCtrl *gfsc = dynamic_cast<GuiFrameSetCtrl *>(obj);
   if (gfsc != NULL)
   {
      Point2I extent(getMax(0, dAtoi(argv[3])), getMax(0, dAtoi(argv[4])));
      gfsc->frameMinExtent(dAtoi(argv[2]), extent);
   }
}

static void cGFSCAddColumn(SimObject * obj, S32, const char **)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   Vector<S32> * columns = ctrl->columnOffsets();
   columns->push_back(0);
   ctrl->balanceFrames();
}

static void cGFSCAddRow(SimObject * obj, S32, const char **)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
...
static void cGFSCRemoveColumn(SimObject * obj, S32, const char **)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   Vector<S32> * columns = ctrl->columnOffsets();
...
static void cGFSCRemoveRow(SimObject * obj, S32, const char **)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   Vector<S32> * rows = ctrl->rowOffsets();
...
static S32 cGFSCGetColumnCount(SimObject * obj, S32, const char **)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   return(ctrl->columnOffsets()->size());
}

static S32 cGFSCGetRowCount(SimObject * obj, S32, const char **)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   return(ctrl->rowOffsets()->size());
}

static S32 cGFSCGetColumnOffset(SimObject * obj, S32, const char ** argv)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   S32 index = dAtoi(argv[2]);
   if(index < 0 || index > ctrl->columnOffsets()->size())
   {
      Con::errorf(ConsoleLogEntry::General, "Column index out of range");
      return(0);
   }
   return((*ctrl->columnOffsets())[index]);
}

static S32 cGFSCGetRowOffset(SimObject * obj, S32, const char ** argv)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
   S32 index = dAtoi(argv[2]);
   if(index < 0 || index > ctrl->rowOffsets()->size())
   {
      Con::errorf(ConsoleLogEntry::General, "Row index out of range");
      return(0);
   }
   return((*ctrl->rowOffsets())[index]);
}

static void cGFSCSetColumnOffset(SimObject * obj, S32, const char ** argv)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
...
static void cGFSCSetRowOffset(SimObject * obj, S32, const char ** argv)
{
   GuiFrameSetCtrl * ctrl = dynamic_cast<GuiFrameSetCtrl*>(obj);
...
//-----------------------------------------------------------------------------
void GuiFrameSetCtrl::consoleInit()
{
   Con::addCommand("GuiFrameSetCtrl",   "frameBorder",    cGFSCFrameBorderEnable,    "gfsc.frameBorder(index, enable)",  3, 4);
   Con::addCommand("GuiFrameSetCtrl",   "frameMovable",   cGFSCFrameBorderMovable,   "gfsc.frameMovable(index, enable)", 3, 4); 
   Con::addCommand("GuiFrameSetCtrl",   "frameMinExtent", cGFSCFrameMinExtent,       "gfsc.frameMinExtent(index, w, h)", 5, 5);
   Con::addCommand("GuiFrameSetCtrl",   "addColumn",      cGFSCAddColumn,            "gfsc.addColumn();",                2, 2);
   Con::addCommand("GuiFrameSetCtrl",   "addRow",         cGFSCAddRow,               "gfsc.addRow();",                   2, 2);
   Con::addCommand("GuiFrameSetCtrl",   "removeColumn",   cGFSCRemoveColumn,         "gfsc.removeColumn();",             2, 2);
   Con::addCommand("GuiFrameSetCtrl",   "removeRow",      cGFSCRemoveRow,            "gfsc.removeRow();",                2, 2);
   Con::addCommand("GuiFrameSetCtrl",   "getColumnCount", cGFSCGetColumnCount,       "gfsc.getColumnCount();",           2, 2);
   Con::addCommand("GuiFrameSetCtrl",   "getRowCount",    cGFSCGetRowCount,          "gfsc.getRowCount();",              2, 2);
   Con::addCommand("GuiFrameSetCtrl",   "getColumnOffset",cGFSCGetColumnOffset,      "gfsc.getColumnOffset(index);",     3, 3);
   Con::addCommand("GuiFrameSetCtrl",   "getRowOffset",   cGFSCGetRowOffset,         "gfsc.getRowOffset(index);",        3, 3);
   Con::addCommand("GuiFrameSetCtrl",   "setColumnOffset",cGFSCSetColumnOffset,      "gfsc.setColumnOffset(index, offset);",  4, 4);
   Con::addCommand("GuiFrameSetCtrl",   "setRowOffset",   cGFSCSetRowOffset,         "gfsc.setRowOffset(index, offset);",     4, 4);
}
...
//-----------------------------------------------------------------------------
void GuiFrameSetCtrl::onMouseMove(const GuiEvent &event)
{
...
//-----------------------------------------------------------------------------
void GuiFrameSetCtrl::onRender(Point2I offset, const RectI &updateRect, GuiControl *firstResponder)
{
