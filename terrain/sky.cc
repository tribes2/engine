...
//--------------------------------------------------------------------------- 
static void cStormCloudsOn(SimObject *obj, S32, const char **argv)
{
   Sky *ctrl = static_cast<Sky*>(obj);
   ctrl->stormCloudsOn(dAtoi(argv[2]), dAtof(argv[3]));
}

static void cStormFogOn(SimObject *obj, S32, const char **argv)
{
   Sky *ctrl = static_cast<Sky*>(obj);
   ctrl->stormFogOn(dAtof(argv[2]), dAtof(argv[3]));
}

static void cStormRealFog(SimObject *obj, S32, const char **argv)
{
   Sky *ctrl = static_cast<Sky*>(obj);
   ctrl->stormRealFog(dAtoi(argv[2]), dAtof(argv[3]), dAtof(argv[4]), dAtof(argv[5]));
}

static const char * cGetWindVelocity(SimObject *obj, S32, const char**)
{
   Sky * sky = static_cast<Sky*>(obj);
...
static void cSetWindVelocity(SimObject *obj, S32, const char** argv)
{
   Sky * sky = static_cast<Sky*>(obj);
...
static void cStormCloudsShow(SimObject *obj, S32, const char **argv)
{
   Sky *ctrl = static_cast<Sky*>(obj);
   ctrl->stormCloudsShow(dAtob(argv[2]));
}

static void cStormFogShow(SimObject *obj, S32, const char **argv)
{
   Sky *ctrl = static_cast<Sky*>(obj);
...
void Sky::consoleInit()
{
   ...
   Con::addCommand("Sky", "stormClouds", cStormCloudsOn, "sky.stormCloudsOn(0<out> or 1<in>,Time<sec>)",  4, 4);
   Con::addCommand("Sky", "stormFog",    cStormFogOn,    "sky.stormFogOn(Percentage <0 to 1>, Time<sec>)",  4, 4);
   Con::addCommand("Sky", "realFog",     cStormRealFog,  "sky.realFog(0 <off> or 1 <on>, max, min, speed)",  6, 6);
   Con::addCommand("Sky", "getWindVelocity", cGetWindVelocity, "sky.getWindVelocity()", 2, 2);
   Con::addCommand("Sky", "setWindVelocity", cSetWindVelocity, "sky.setWindVelocity(x, y, z)", 5, 5);
   Con::addCommand("Sky", "stormCloudsShow", cStormCloudsShow, "sky.stormCloudsShow(bool)",  3, 3);
   Con::addCommand("Sky", "stormFogShow", cStormFogShow, "sky.stormFogShow(bool)",  3, 3);
}
...
void Sky::initPersistFields()
{
   ...
   addField("materialList",            TypeString,    Offset(mMaterialListName,Sky));
      ...
      if(stream->readFlag())
      {
         ...
         if(mStormFogData.time)
         {
            ...
            {   
               ...
               mStormFogData.volume[x].endPercentage = mStormFogData.endPercentage;
                             ...
                             F32 *alphaBan, F32 depthInFog)
{
   ...
   visDis = setVis = mFogVolumes[mNumFogVolumes-1].visibleDistance;
   for(S32 x = 0; x < mNumFogVolumes-1; ++x)
   {  
      if(mFogVolumes[x].visibleDistance < setVis)
      {
         ...
         if(visValue > 0.0f)
         {
            ratioVal = setVis / mFogVolumes[x].visibleDistance;
            visRatio = visValue / mFogVolumes[x].visibleDistance; 
            visDis -= (mFogVolumes[x].visibleDistance * visRatio) * ratioVal;
         }
      }
   }
...
void Sky::startStormFog()
{
   ...
   if(mFogVolume < 0)
      ...
      {   
         ...
         mStormFogData.volume[x].endPercentage = mFogPercentage;
         if(mStormFogData.volume[x].state == comingIn)   
            mStormFogData.current = 0;
         else
            mStormFogData.current = mNumFogVolumes-1;
