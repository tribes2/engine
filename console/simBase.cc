...
namespace Sim
{
   ...
   ImplementNamedSet(ClientTargetSet)
   ImplementNamedSet(ServerTargetSet)
   ImplementNamedSet(FlareSet)
   ImplementNamedSet(MissileSet)
   ImplementNamedSet(CommandTargetSet)
   ImplementNamedSet(ScopeSensorVisibleSet)
...
void SimObject::assignFieldsFrom(SimObject *parent)
{
   ...
   if(getClassRep() == parent->getClassRep())
   {
      ...
      for(U32 i = 0; i < list.size(); i++)
      {
         ...
         if(f->elementCount == 1)
         {
            const char *fieldVal = Con::getData(f->type, (void *) (U32(parent) + f->offset), 0, f->table, f->flag);
            if(fieldVal)
               Con::setData(f->type, (void *) (U32(this) + f->offset), 0, 1, &fieldVal, f->table);
         }
         else
         {
            for(U32 j = 0; j < f->elementCount; j++)
            {
               const char *fieldVal = Con::getData(f->type, (void *) (U32(parent) + f->offset), j, f->table, f->flag);
               if(fieldVal)
                  Con::setData(f->type, (void *) (U32(this) + f->offset), j, 1, &fieldVal, f->table);
            }         
         }
      }
   }
   ...
   for(U32 i = 0; i < list.size(); i++)
   {
      ...
      for(U32 j = 0; S32(j) < f->elementCount; j++)
      {
         if(f->type == AbstractClassRep::DepricatedFieldType)
            continue;
         const char *val = Con::getData(f->type, (void *) (U32(this) + f->offset), j, f->table, f->flag);
         if(!val || !*val)
            continue;
         if(f->elementCount == 1)
...
ConsoleMethod(SimObject, save, bool, 3, 4, "obj.save(fileName, <selectedOnly>)")
{
   ...
   if(!ResourceManager->openFileForWrite(stream, ResourceManager->getModPathOf(argv[2]), argv[2]))
      return false;
...
ConsoleFunction(schedule, S32, 4, 0, "object.schedule(time, command, <arg1...argN>)")
{
...
ConsoleMethod(SimObject,dump, void, 2, 2, "obj.dump()")
{
   ...
   for(Vector<const AbstractClassRep::Field *>::iterator itr = flist.begin(); itr != flist.end(); itr++)
   {
      ...
      for(U32 j = 0; S32(j) < f->elementCount; j++)
      {
         if(f->type == AbstractClassRep::DepricatedFieldType)
            continue;
         const char *val = Con::getData(f->type, (void *) (U32(object) + f->offset), j, f->table, f->flag);
         if(!val || !*val)
            continue;
         if(f->elementCount == 1)
            dSprintf(expandedBuffer, sizeof(expandedBuffer), "  %s = \"", f->pFieldname);
...
void SimObject::consoleInit()
{
   ...
   Con::setIntVariable("$TypeMasks::ForceFieldObjectType",     ForceFieldObjectType);
   ...
   Con::setIntVariable("$TypeMasks::GuiControlObjectType",     GuiControlObjectType);
   ...
   Con::setIntVariable("$TypeMasks::SensorObjectType",         SensorObjectType);
   Con::setIntVariable("$TypeMasks::StationObjectType",        StationObjectType);
   Con::setIntVariable("$TypeMasks::GeneratorObjectType",      GeneratorObjectType);
}
   ...
   if(mFlags.test(ModStaticFields))
   {
      ...
      if(fld)
      {
         ...
         if(array1 >= 0 && array1 < fld->elementCount && fld->elementCount >= 1)
            Con::setData(fld->type, (void *) (U32(this) + fld->offset), array1, 1, &value, fld->table);
   ...
   if(mFlags.test(ModStaticFields))
   {
      ...
      if(fld)
      {
         if(array1 == -1 && fld->elementCount == 1)
            return Con::getData(fld->type, (void *) (U32(this) + fld->offset), 0, fld->table, fld->flag);
         if(array1 >= 0 && array1 < fld->elementCount)
            return Con::getData(fld->type, (void *) (U32(this) + fld->offset), array1, fld->table, fld->flag);// + typeSizes[fld.type] * array1));
         return "";
      }
   }
   if(mFlags.test(ModDynamicFields))
...
bool SimDataBlock::onAdd()
{
   ...
   AssertFatal(sNextObjectId <= DataBlockObjectIdLast,
               "Exceeded maxmimum number of data blocks");
...
ConsoleFunction(deleteDataBlocks, void, 1, 1, "deleteDataBlocks();")
{
   argc; argv;
...
{
   argc; argv;
   SimSet *set = (SimSet *) object;
   SimSet::iterator itr;
   for(itr = set->begin(); itr != set->end(); itr++)
   {
      SimObject *object = *itr;
      bool isSet = dynamic_cast<SimSet *>(object) != 0;
      const char *name = object->getName();
      if(name)
         Con::printf("   %d,\"%s\": %s %s", object->getId(), name,
                     object->getClassName(), isSet ? "(g)":"");
      else
         Con::printf("   %d: %s %s", object->getId(), object->getClassName(),
                     isSet ? "(g)" : "");
   }
}
...
ConsoleMethod(SimSet, remove, void, 3, 0, "set.remove(obj1,...)")
{
   ...
   {
      SimObject *obj = Sim::findObject(argv[i]);
      if(obj && set->find(set->begin(),set->end(),obj) != set->end())
...
ConsoleMethod(SimSet, getObject, S32, 3, 3, "set.getObject(objIndex)")
{
   ...
   if(objectIndex < 0 || objectIndex >= S32(set->size()))
   {
      Con::printf("Set::getObject index out of range.");
      return -1;
   }
   return ((*set)[objectIndex])->getId();
}
...
ConsoleMethod(SimSet, bringToFront, void, 3, 3, "set.bringToFront(object)")
{
   ...
   if(!obj)
      return;
   ((SimSet *) object)->bringObjectToFront(obj);
}
...
ConsoleMethod(SimSet, pushToBack, void, 3, 3, "set.pushToBack(object)")
{
   ...
   if(!obj)
      return;
   ((SimSet *) object)->pushObjectToBack(obj);
}
