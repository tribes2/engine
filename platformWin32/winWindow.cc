...
const char* Platform::getLoginPassword()
{
   ...
   if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, Tribes2RegKey, 0, KEY_QUERY_VALUE, &regKey ) == ERROR_SUCCESS )
   {
      U8 buf[32];
...
{
   HKEY regKey;
   if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, Tribes2RegKey, 0, KEY_WRITE, &regKey ) == ERROR_SUCCESS )
   {
