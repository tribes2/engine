...
bool Interior::castRay(const Point3F& s, const Point3F& e, RayInfo* info)
{
   ...
   AssertFatal(!hit || (info->normal.z == info->normal.z), "NaN returned from castRay.  Please talk to DMoore if you are running this in the debugger");
                         ...
                         U32*           numZones)
{
   static InteriorPolytope pTope;
   pTope.clear();
   pTope.buildBox(box, transform, Point3F(1, 1, 1));

   scanZoneNew(pTope, zones, numZones);
...
Point3F InteriorConvex::support(const VectorF& v) const
{
   ...
   if (hullId >= 0)
   {
      ...
      F32* pDots = (F32*)FrameAllocator::alloc(sizeof(F32) * rHull.hullCount);
...
void InteriorConvex::getFeatures(const MatrixF& mat, const VectorF& n, ConvexFeature* cf)
{
   ...
   if (hullId >= 0)
   {
      ...
      U32* pRemaps = (U32*)FrameAllocator::alloc(256 * sizeof(U32));
      ...
      // Reset the frame allocator.  It's work is done...
   ...
   else
   {
      ...
      //  vertex...
      U32 currPos = 0;
      const U8* pString = &pInterior->mVehicleConvexHullEmitStrings[pInterior->mVehicleHullEmitStringIndices[rHull.hullStart +
                                                                                                             spIndex]];
...
void InteriorConvex::getPolyList(AbstractPolyList* list)
{
   ...
   if (hullId >= 0)
   {
      ...
      U32 waterMark        = FrameAllocator::getWaterMark();
      U32* pointRemapTable = reinterpret_cast<U32*>(FrameAllocator::alloc(numPoints * sizeof(U32)));
      {
      ...
      // Reset the watermark on the allocator
      {
         FrameAllocator::setWaterMark(waterMark);
      }
   }
