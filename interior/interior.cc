...
void fixupViewport(PortalRenderInfo& rInfo)
{
   ...
   if ((cdV1 - fdV0) <= 1.0)
   {
      if (fdV0 == sgStoredViewport.point.x)
      {
         // Up against the left side
         cdV1 += 2.0;
      }
      else if (cdV1 == sgStoredViewport.point.x + sgStoredViewport.extent.x)
      {
         // Up against the right side
         fdV0 -= 2.0;
      }
      else
      {
   ...
   if ((cdV1 - fdV0) <= 1.0)
   {
      if (fdV0 == sgStoredViewport.point.y)
      {
         // Up against the top
         cdV1 += 2.0;
      }
      else if (cdV1 == sgStoredViewport.point.y + sgStoredViewport.extent.y)
      {
...
bool Interior::prepForRendering()
{
   ...
   bool matListSuccess = mMaterialList->load(InteriorTexture, false);
   ...
   mWhite        = new TextureHandle("special/whiteAlpha255", MeshTexture);
   mWhiteRGB     = new TextureHandle("special/whiteNoAlpha", MeshTexture);
   mLightFalloff = new TextureHandle("special/lightFalloffMono", BitmapTexture, true);
